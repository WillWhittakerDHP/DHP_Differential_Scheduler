...
### 10.1a Storage column names

The **tier table** above is also the persisted contract. In **PostgreSQL**, **Sequelize**, API payloads, snapshots, and booking/admin client types, the PerUnit columns are named **`timePerUnit`** and **`feePerUnit`** on `part_instances` and `part_instance_versions`. **`baseTime`** and **`baseFee`** remain the Base tier columns.
...
**Implemented booking path (close-out):** `createPartFinals` emits **one `PartFinal` per `partInstance.id`**. `buildAppointmentShape` builds **`eventAssignmentsByPartInstanceId`** (keys = part instance ids) from **`eventAssignments`** edges whose parent is the owning **block instance** (baseline) or that **part instance** (override). Global hydration groups `event_assignments` by API `parent_kind` so part-scoped parents resolve in `transformApiRelationships`.
...
- **Admin panel (instances):** Top-level **Orchestration**, **Services**, and **Scheduling** tabs filter block **shapes** by type (`user` / `service` / `time+event+price`) so instance editing stays domain-scoped; each tab reuses the shared instances machinery with an `allowedBlockShapeTypes` filter (`AdminPanel.vue` → `OrchestrationTab` / `ServicesTab` / `SchedulingBlockInstancesTab` → `InstancesTab`).
...
3. **Part instances are per-block-instance with two resolution tiers:** Own part sets via `part_assignments`; no cross-writes.
   ...
   - **3d.** **Lineage:** PartFinalizer must not use `part_shape` alone when multiple logical work items could collide; slot-shape aggregation uses **`partInstance.id`** keys in **`eventAssignmentsByPartInstanceId`** (§10.2).
