...
export function getFieldMetadata<GE extends GlobalEntityKey, FieldKey extends GlobalFieldKey<GE>>(
  ...
  const displayProps: DisplayProps = (() => {
    ...
    return DISPLAY_FIELD_KEYS.reduce<DisplayProps>((acc, key) => {
      if (key === 'helpText') {
        const v = (actualDisplayConfig?.helpText ?? actualDisplayConfig?.tooltip) as string | undefined
        if (v == null) {
          return acc
        }
        if (typeof v === 'string') {
      ...
      if (key === 'required' || key === 'disabled' || key === 'readOnly') {
        const meta = actualDisplayConfig?.meta as Record<string, unknown> | undefined
        const v = (meta?.[key] ?? actualDisplayConfig?.[key]) as boolean | undefined
        if (v == null) {
          return acc
        }
        if (typeof v === 'boolean') {
      ...
      const v = actualDisplayConfig?.[key]
      if (v == null) {
        return acc
      }
      if (typeof v === 'string') {
