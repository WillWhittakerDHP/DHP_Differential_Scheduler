...
export function useBlockInstanceForm(
  ...
  const loadEntity = async (): Promise<void> => {
    ...
    const entity = getGlobalEntities('blockInstance').find(e => String(e.id) === entityId.value)
    if (entity) {
      formData.value = {
        name: entity.name || '',
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    try {
      if (isEdit.value && entityId.value) {
        await update(formData.value as Partial<GlobalEntity<'blockInstance'>>, entityId.value)
      } else {
