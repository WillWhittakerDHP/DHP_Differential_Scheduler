...
import { createActivePartsRel } from '@/utils/__tests__/factories/relationshipFactory'
...
const relationship = createActivePartsRel('block-1', ['part-1', 'part-2'])
mockGlobalData.relationships.activeParts = [relationship]
...
describe('useRelationshipCrud', () => {
  ...
  describe('relationships computed', () => {
    ...
    it('should transform GlobalRelationship to FetchedRelationship format', () => {
      ...
      expect(rel.kind).toBe('activeParts')
    })
  ...
  describe('isLoading and error', () => {
    it('should always return false for isLoading', () => {
      const { isLoading } = useRelationshipCrud('activeParts')
      
      expect(isLoading.value).toBe(false)
    })

    it('should always return undefined for error', () => {
      const { error } = useRelationshipCrud('activeParts')
  ...
  describe('create mutation', () => {
    it('should create relationship', async () => {
      ...
      const { create } = useRelationshipCrud('activeParts')
  ...
  describe('remove mutation', () => {
    it('should delete relationship', async () => {
      ...
      const { remove } = useRelationshipCrud('activeParts')
      
      await remove('block-1', 'part-1')
      
      expect(apiClient.delete).toHaveBeenCalledWith(
        '/api/relationships/activeParts/block-1/part-1'
      )
    })
  ...
  describe('refetch', () => {
    it('should refetch globalData', async () => {
      const { refetch } = useRelationshipCrud('activeParts')
