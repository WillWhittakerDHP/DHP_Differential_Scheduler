...
describe('appointmentToWizardTransformer', () => {
  describe('transformAppointmentToWizard', () => {
    it('should transform appointment with single service', async () => {
      ...
      const transformer = bookingTransformer
      ...
      expect(result.availability.candidateDate.start).toBe('2026-01-15')
      expect(result.availability.candidateTimeSlots).toBeDefined()
    })
    ...
    it('should create time slots from appointment selectedTimeSlots', async () => {
      ...
      expect(result.availability.candidateTimeSlots).toBeDefined()
      expect(result.availability.candidateTimeSlots).toHaveLength(1)
      expect(result.availability.candidateTimeSlots?.[0].time).toBe('2026-01-15T10:00:00Z')
      expect(result.availability.candidateTimeSlots?.[0].duration).toBe(120)
    })
