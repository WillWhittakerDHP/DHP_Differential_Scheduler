        ...
        <v-card>
          ...
          <v-card-text>
            ...
            <v-list v-if="!isLoading && entities.length > 0">
              ...
              >
                <v-list-item-title>{{ entity.name || `Part Instance ${entity.id}` }}</v-list-item-title>
                <v-list-item-subtitle>
                  ID: {{ entity.id }} | Order: {{ entity.orderIndex }} | 
                  Part Shape: {{ entity.partShapeRef || 'N/A' }}
                </v-list-item-subtitle>
            ...
            <v-alert
              ...
              type="info"
            >
              No part instances found. Create one to get started.
            </v-alert>
          </v-card-text>
        </v-card>
      </v-col>
...
const handleDelete = entityListDelete({
  remove,
  confirmMessage: 'Are you sure you want to delete this part instance?',
  errorMessage: 'Failed to delete part instance',
  logger,
  notifyError,
})

function goToCreate(): void {
  router.push({ name: 'part-instance-create' })
}

function goToEdit(id: GlobalEntityId): void {
  router.push({ name: 'part-instance-edit', params: { id: String(id) } })
}
</script>
