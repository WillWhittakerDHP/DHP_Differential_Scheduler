...
const outOfOfficeEvents = computed(() => {
  if (!props.computedAvailability) return []
  return props.computedAvailability.calendarEvents.value.filter(
    (e: SharedCalendarEvent) => e.eventType === 'outOfOffice'
  )
})
</script>
...
<template>
  <div class="pa-3">
    <div v-if="!computedAvailability" class="text-center pa-4">
      ...
      <p class="text-body-medium">API Orchestrator not available. This tab is only available in the booking wizard.</p>
    </div>
    ...
    <div v-else>
      ...
      <div v-if="computedAvailability.isLoading.value" class="text-center pa-4">
        ...
        <p class="text-body-medium mt-2">Loading computed availability data...</p>
      </div>
      ...
      <div v-else-if="computedAvailability.error.value" class="text-center pa-4">
        ...
        <p class="text-body-medium text-error">{{ computedAvailability.error.value.message }}</p>
      </div>
      ...
      <div v-else>
        <!-- Constraints Summary -->
        <div class="mb-4">
          <h3 class="text-body-large font-weight-bold mb-2">Constraints</h3>
        ...
        <div class="mb-4">
          ...
          <VCard variant="outlined" class="pa-2" style="max-height: 200px; overflow-y: auto;">
            <div v-if="computedAvailability.calendarEvents.value.length === 0" class="text-center pa-2 text-body-small text-medium-emphasis">
              No calendar events
            </div>
            <div v-else>
              <div
              ...
              >
                <div class="text-body-small font-weight-bold">{{ event.summary || '(No title)' }}</div>
                <div class="text-body-small text-medium-emphasis">
                  {{ formatDateTimeForDisplay(event.start as RFC3339DateTime) }} - {{ formatTimeForDisplay(event.end as RFC3339DateTime) }}
                </div>
                <div v-if="event.placeId" class="text-body-small text-medium-emphasis">
              ...
              <div v-if="computedAvailability.calendarEvents.value.length > 10" class="text-center pa-2 text-body-small text-medium-emphasis">
        ...
        <div class="mb-4" v-if="outOfOfficeEvents.length > 0">
          <h3 class="text-body-large font-weight-bold mb-2">
            Out-of-Office Events
          ...
          <VCard variant="outlined" class="pa-2" style="max-height: 150px; overflow-y: auto;">
            <div
              v-for="(event, idx) in outOfOfficeEvents"
              :key="idx"
              class="mb-2 pa-2"
        ...
        <div class="mb-4">
          ...
          <VCard variant="outlined" class="pa-2" style="max-height: 200px; overflow-y: auto;">
            <div v-if="slotsByDaySummary.dayCount === 0" class="text-center pa-2 text-body-small text-medium-emphasis">
              No slots cached yet
            </div>
            <div v-else>
              <div
              ...
              <div v-if="slotsByDaySummary.dayCount > 10" class="text-center pa-2 text-body-small text-medium-emphasis">
