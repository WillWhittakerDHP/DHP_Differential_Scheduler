 ...
 *
 * Output:
 * - client/.audit-reports/loop-mutation-audit.json
 * - client/.audit-reports/loop-mutation-audit.md
 *
 * Exception Handling:
 * - Inline: // @audit-allow:loop-mutation:<ruleId> - <reason>
 * - Config: .audit-reports/loop-mutation-audit-config.json (allowlist patterns/specific)
 *
 * Notes:
...
function renderMarkdownReport(files, exceptionSummary) {
  ...
  lines.push('- Config: `.audit-reports/loop-mutation-audit-config.json`')
  lines.push('')
  lines.push('## Summary')
  lines.push('')
