 ...
 *
 * Output:
 * - client/.audit-reports/duplication-audit.json
 * - client/.audit-reports/duplication-audit.md
 *
 * Notes:
...
function renderMarkdownReport(data) {
  ...
  lines.push('- Excluded: See `.audit-reports/duplication-audit-config.json` for exclusion patterns')
  lines.push('- Vue scanning: `<script>` blocks only (templates/styles excluded)')
  lines.push('')
  lines.push('Exception handling:')
  lines.push('- Config: `.audit-reports/duplication-audit-config.json` (allowlist patterns/specific)')
  lines.push('')
