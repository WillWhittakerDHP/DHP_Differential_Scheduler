...
const AUDIT_JSON = path.join(AUDIT_DIR, 'lint-audit.json')
const OUT_MD = path.join(AUDIT_DIR, 'lint-audit-summary.md')
...
function main() {
  ...
  const files = Array.isArray(data.files) ? data.files : []
  console.log(`Wrote: lint-audit-summary.md (files with findings: ${files.length})`)
  process.exitCode = 0
}
