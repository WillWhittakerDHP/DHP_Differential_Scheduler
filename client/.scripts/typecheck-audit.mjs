 ...
 *
 * Mirrors the `client/.audit-reports/` workflow:
 ...
 * - Config: .audit-reports/typecheck/typecheck-audit-config.json (allowlist patterns/specific)
 *
 * Output:
 * - client/.audit-reports/typecheck/typecheck-audit.json
 * - client/.audit-reports/typecheck/typecheck-audit.md
 */
...
function checkErrorAllowed(error, config, inlineExceptions) {
  ...
  // Check config-based allowlist (central allowlist uses ruleIds/ruleId; error.code is the rule id)
  const allowlist = config?.allowlist
  if (!allowlist) {
    return { allowed: false, reason: null, source: null }
  }

  // Pattern-based allowances (ruleIds in central allowlist)
  for (const pattern of (allowlist.patterns || [])) {
    ...
    if (globMatch) {
      ...
      if (ruleIds.includes(error.code) || ruleIds.includes('*')) {
        return { allowed: true, reason: pattern.reason, source: 'pattern' }
      }
    }
  }

  // Specific file/line allowances (ruleId in central allowlist)
  for (const specific of (allowlist.specific || [])) {
    ...
    if (fileMatch && (specific.ruleId === error.code || specific.ruleId === '*')) {
...
function renderMarkdown(data) {
  ...
  lines.push('- Config: `.audit-reports/typecheck/typecheck-audit-config.json` (allowlist section)')
  lines.push('')
  lines.push('## Summary')
  lines.push('')
...
function main() {
  ...
  // Filter out paths covered by central global exclusions (migrations, seed, tests, etc.)
