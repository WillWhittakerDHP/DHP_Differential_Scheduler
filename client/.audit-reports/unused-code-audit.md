# Unused Code Audit (Generated)

This file is generated by `client/.scripts/unused-code-audit.mjs`.

Scope: `client/src/**/*.{ts,js,vue}`

## Summary

- Files scanned: **672**
- Files with issues: **232**
- Issues found: **455**
- Unused exports: 402
- Commented exports: 0
- Unused functions: 50
- TODO markers: 3

## Issues by File (sorted by priority)

### `src/composables/booking/useBlockInstanceSelection.ts` [P0] (score: 15)

- **WARNING** [unused-export] (line 17): Unused export: SelectionMode (type)
  ```
  export type SelectionMode = 'single' | 'multiple'
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 22): Unused export: UseBlockInstanceSelectionParams (type)
  ```
  export interface UseBlockInstanceSelectionParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 32): Unused export: UseBlockInstanceSelectionReturnSingle (type)
  ```
  export interface UseBlockInstanceSelectionReturnSingle {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 39): Unused export: UseBlockInstanceSelectionReturnMultiple (type)
  ```
  export interface UseBlockInstanceSelectionReturnMultiple {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 32): Unused export: UseBlockInstanceSelectionReturn (type)
  ```
  export interface UseBlockInstanceSelectionReturnSingle {
  ```
  💡 Remove if unused or document why kept

### `src/types/appointment.ts` [P0] (score: 15)

- **WARNING** [unused-export] (line 124): Unused export: TimeSlotKind (type)
  ```
  export type TimeSlotKind =
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 213): Unused export: AvailabilityRequest (type)
  ```
  export interface AvailabilityRequest {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 228): Unused export: AvailabilityResponse (type)
  ```
  export interface AvailabilityResponse {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 237): Unused export: PartInstanceSnapshot (type)
  ```
  export interface PartInstanceSnapshot {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 251): Unused export: BlockInstanceSnapshot (type)
  ```
  export interface BlockInstanceSnapshot {
  ```
  💡 Remove if unused or document why kept

### `src/utils/blockInstanceUtils.ts` [P0] (score: 15)

- **WARNING** [unused-export] (line 25): Unused export: getBlockInstancesByShapeId (function)
  ```
  export function getBlockInstancesByShapeId(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 44): Unused export: getBlockInstancesByShapeIdFromBooking (function)
  ```
  export function getBlockInstancesByShapeIdFromBooking(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 92): Unused export: getStateControlBlockShapes (function)
  ```
  export function getStateControlBlockShapes(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 172): Unused export: getBlockInstancesByShapeName (function)
  ```
  export function getBlockInstancesByShapeName(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 214): Unused export: getBlockInstancesByType (function)
  ```
  export function getBlockInstancesByType(
  ```
  💡 Remove if unused or document why kept

### `src/utils/booking/timeSlotFitter.ts` [P0] (score: 15)

- **WARNING** [unused-export] (line 220): Unused export: isClosedDay (function)
  ```
  export function isClosedDay(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 46): Unused export: DayBusinessHours (type)
  ```
  export interface DayBusinessHours {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 83): Unused export: FitTimeSlotsParams (type)
  ```
  export interface FitTimeSlotsParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 122): Unused export: FitTimeSlotsResult (type)
  ```
  export interface FitTimeSlotsResult {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 437): Unused export: FitTimeSlotsResultWithAvailability (type)
  ```
  export interface FitTimeSlotsResultWithAvailability {
  ```
  💡 Remove if unused or document why kept

### `src/utils/booking/timeAvailabilityManager.ts` [P0] (score: 13)

- **WARNING** [unused-export] (line 753): Unused export: clearScheduledHoursCache (function)
  ```
  export function clearScheduledHoursCache(): void {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 898): Unused export: markSlotAvailability (function)
  ```
  export function markSlotAvailability(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 240): Unused export: AvailabilityManagerResult (type)
  ```
  export interface AvailabilityManagerResult {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 248): Unused export: GenerateSlotsWithAvailabilityParams (type)
  ```
  export interface GenerateSlotsWithAvailabilityParams {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 122): Unused function: validateBusyPeriod
  ```
  function validateBusyPeriod(busy: BusyTimeRange): boolean {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useEntityCardReadiness.ts` [P0] (score: 12)

- **WARNING** [unused-export] (line 20): Unused export: isEntityCardReady (function)
  ```
  export function isEntityCardReady<GE extends GlobalEntityKey>(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 38): Unused export: getFieldContextForInstance (function)
  ```
  export function getFieldContextForInstance<GE extends GlobalEntityKey>(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 61): Unused export: getFieldContextForPartInstance (function)
  ```
  export function getFieldContextForPartInstance<GE extends GlobalEntityKey>(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 77): Unused export: useEntityCardReadiness (const)
  ```
  export const useEntityCardReadiness = {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useTimeSlotCalculations.ts` [P0] (score: 12)

- **WARNING** [unused-export] (line 20): Unused export: TimeBlock (type)
  ```
  export interface TimeBlock {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 29): Unused export: TimeOnSiteBlocks (type)
  ```
  export interface TimeOnSiteBlocks {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 37): Unused export: UseTimeSlotCalculationsParams (type)
  ```
  export interface UseTimeSlotCalculationsParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 49): Unused export: UseTimeSlotCalculationsReturn (type)
  ```
  export interface UseTimeSlotCalculationsReturn {
  ```
  💡 Remove if unused or document why kept

### `src/configs/field/display/fullFieldDisplayConfig.ts` [P0] (score: 12)

- **WARNING** [unused-export] (line 102): Unused export: buildAllPerEntityDisplayConfig (function)
  ```
  export function buildAllPerEntityDisplayConfig<GE extends GlobalEntityKey>(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 153): Unused export: buildDisplayFieldType (function)
  ```
  export function buildDisplayFieldType(): DisplayFieldTypeMap {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 52): Unused export: DisplayFieldTypeMap (type)
  ```
  export type DisplayFieldTypeMap = {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 62): Unused export: DisplayFieldAdmin (type)
  ```
  export interface DisplayFieldAdmin<GE extends GlobalEntityKey, FieldKey extends GlobalFieldKey<GE>> {
  ```
  💡 Remove if unused or document why kept

### `src/views/admin/tabs/ShapesTab.vue` [P0] (score: 10)

- **INFO** [unused-function] (line 124): Unused function: handlePartInstanceMetadataSaved
  ```
  const handlePartInstanceMetadataSaved = () => {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 162): Unused function: createAnnotationShape
  ```
  const createAnnotationShape = () => {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 171): Unused function: handlePartShapeCreated
  ```
  const handlePartShapeCreated = (_entity: GlobalEntity<GlobalEntityKey>) => {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 180): Unused function: handlePartShapeCancelled
  ```
  const handlePartShapeCancelled = () => {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 189): Unused function: handleAnnotationShapeCreate
  ```
  const handleAnnotationShapeCreate = async () => {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 206): Unused function: handleAnnotationShapeCancelled
  ```
  const handleAnnotationShapeCancelled = () => {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 334): Unused function: handleDeletePartShape
  ```
  function handleDeletePartShape(_id: string) {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 345): Unused function: handleDeleteBlockShape
  ```
  function handleDeleteBlockShape(_id: string) {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 379): Unused function: handleDeleteAnnotationShape
  ```
  function handleDeleteAnnotationShape(_id: string) {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 389): Unused function: handleExistingShapeSaved
  ```
  function handleExistingShapeSaved(entity: GlobalEntity<GlobalEntityKey>) {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useAnnotationDisplay.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 21): Unused export: AnnotationDisplay (type)
  ```
  export interface AnnotationDisplay {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 32): Unused export: UseAnnotationDisplayOptions (type)
  ```
  export interface UseAnnotationDisplayOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 47): Unused export: UseAnnotationDisplayReturn (type)
  ```
  export interface UseAnnotationDisplayReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useBlockInstanceForm.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 24): Unused export: BlockInstanceFormData (type)
  ```
  export interface BlockInstanceFormData {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 34): Unused export: UseBlockInstanceFormOptions (type)
  ```
  export interface UseBlockInstanceFormOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 46): Unused export: UseBlockInstanceFormReturn (type)
  ```
  export interface UseBlockInstanceFormReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useDialogFormState.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 52): Unused export: useDialogFormState (function)
  ```
  export function useDialogFormState<T extends Record<string, unknown> = Record<string, unknown>>(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 18): Unused export: UseDialogFormStateOptions (type)
  ```
  export interface UseDialogFormStateOptions<T extends Record<string, unknown> = Record<string, unknown>> {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 33): Unused export: UseDialogFormStateReturn (type)
  ```
  export interface UseDialogFormStateReturn<T extends Record<string, unknown> = Record<string, unknown>> {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useDragAndDrop.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 18): Unused export: DragEndHandler (type)
  ```
  export type DragEndHandler = () => Promise<void>
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 23): Unused export: UseDragAndDropParams (type)
  ```
  export interface UseDragAndDropParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 37): Unused export: UseDragAndDropReturn (type)
  ```
  export interface UseDragAndDropReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useEntityCardLayout.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 81): Unused export: useEntityCardLayout (function)
  ```
  export function useEntityCardLayout(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 34): Unused export: UseEntityCardLayoutOptions (type)
  ```
  export interface UseEntityCardLayoutOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 55): Unused export: UseEntityCardLayoutReturn (type)
  ```
  export interface UseEntityCardLayoutReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useEntityGrouping.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 55): Unused export: useEntityGrouping (function)
  ```
  export function useEntityGrouping<
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 17): Unused export: UseEntityGroupingParams (type)
  ```
  export interface UseEntityGroupingParams<
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 41): Unused export: UseEntityGroupingReturn (type)
  ```
  export interface UseEntityGroupingReturn<EntityKey extends GlobalEntityKey> {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useFieldMetadataUpdate.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 33): Unused export: useFieldMetadataUpdate (function)
  ```
  export function useFieldMetadataUpdate(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 13): Unused export: FieldMetadataConfig (type)
  ```
  export interface FieldMetadataConfig {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 21): Unused export: FieldVisibilityConfig (type)
  ```
  export type FieldVisibilityConfig = FieldMetadataConfig
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useInputConfigEditor.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 12): Unused export: InputConfigFormData (type)
  ```
  export interface InputConfigFormData {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 22): Unused export: UseInputConfigEditorOptions (type)
  ```
  export interface UseInputConfigEditorOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 27): Unused export: UseInputConfigEditorReturn (type)
  ```
  export interface UseInputConfigEditorReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/usePartInstanceForm.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 24): Unused export: PartInstanceFormData (type)
  ```
  export interface PartInstanceFormData {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 34): Unused export: UsePartInstanceFormOptions (type)
  ```
  export interface UsePartInstanceFormOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 46): Unused export: UsePartInstanceFormReturn (type)
  ```
  export interface UsePartInstanceFormReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useShapeCreation.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 32): Unused export: useShapeCreation (function)
  ```
  export function useShapeCreation(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 11): Unused export: UseShapeCreationReturn (type)
  ```
  export interface UseShapeCreationReturn {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 23): Unused export: UseShapeCreationOptions (type)
  ```
  export interface UseShapeCreationOptions {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useShapeSaveHandlers.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 38): Unused export: useShapeSaveHandlers (function)
  ```
  export function useShapeSaveHandlers(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 13): Unused export: UseShapeSaveHandlersOptions (type)
  ```
  export interface UseShapeSaveHandlersOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 23): Unused export: UseShapeSaveHandlersReturn (type)
  ```
  export interface UseShapeSaveHandlersReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useStatusButtonHandlers.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 28): Unused export: useStatusButtonHandlers (function)
  ```
  export function useStatusButtonHandlers<GE extends GlobalEntityKey>(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 13): Unused export: UseStatusButtonHandlersOptions (type)
  ```
  export interface UseStatusButtonHandlersOptions<GE extends GlobalEntityKey> {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 18): Unused export: UseStatusButtonHandlersReturn (type)
  ```
  export interface UseStatusButtonHandlersReturn<GE extends GlobalEntityKey> {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useAppointmentTimes.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 42): Unused export: useAppointmentTimes (function)
  ```
  export function useAppointmentTimes(params: UseAppointmentTimesParams): UseAppointmentTimesReturn {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 18): Unused export: UseAppointmentTimesParams (type)
  ```
  export interface UseAppointmentTimesParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 27): Unused export: UseAppointmentTimesReturn (type)
  ```
  export interface UseAppointmentTimesReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/usePropertyDetailsLogic.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 34): Unused export: SelectionCardItemWithComponents (type)
  ```
  export interface SelectionCardItemWithComponents extends BookingBlockInstance {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 42): Unused export: UsePropertyDetailsLogicParams (type)
  ```
  export interface UsePropertyDetailsLogicParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 55): Unused export: UsePropertyDetailsLogicReturn (type)
  ```
  export interface UsePropertyDetailsLogicReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/fieldContext/useFieldContextSaveHelpers.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 25): Unused export: SaveComponentEntityParams (type)
  ```
  export interface SaveComponentEntityParams<GE extends GlobalEntityKey, FieldKey extends GlobalFieldKey<GE>> {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 88): Unused export: SaveRelationshipFieldParams (type)
  ```
  export interface SaveRelationshipFieldParams<GE extends GlobalEntityKey, FieldKey extends GlobalFieldKey<GE>> {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 182): Unused export: SaveRegularFieldParams (type)
  ```
  export interface SaveRegularFieldParams<GE extends GlobalEntityKey, FieldKey extends GlobalFieldKey<GE>> {
  ```
  💡 Remove if unused or document why kept

### `src/composables/useBusiness.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 69): Unused export: useAppointments (function)
  ```
  export function useAppointments(): ComputedRef<BusinessData['appointments']> {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 81): Unused export: useProperties (function)
  ```
  export function useProperties(): ComputedRef<BusinessData['properties']> {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 93): Unused export: useUsers (function)
  ```
  export function useUsers(): ComputedRef<BusinessData['users']> {
  ```
  💡 Remove if unused or document why kept

### `src/configs/field/display/selectableDisplayConfig.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 32): Unused export: RelationshipDisplayType (type)
  ```
  export type RelationshipDisplayType<
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 81): Unused export: VirtualDisplayType (type)
  ```
  export type VirtualDisplayType<
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 133): Unused export: SelectableDisplayTypeSuite (type)
  ```
  export type SelectableDisplayTypeSuite = {
  ```
  💡 Remove if unused or document why kept

### `src/types/datetime.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 118): Unused export: validateRFC3339DateTime (function)
  ```
  export function validateRFC3339DateTime(value: string): RFC3339DateTime {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 173): Unused export: toDayOfWeek (function)
  ```
  export function toDayOfWeek(n: number): DayOfWeek {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 189): Unused export: getDayOfWeek (function)
  ```
  export function getDayOfWeek(date: Date): DayOfWeek {
  ```
  💡 Remove if unused or document why kept

### `src/types/googleCalendar.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 27): Unused export: GoogleCalendarError (type)
  ```
  export interface GoogleCalendarError {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 38): Unused export: GoogleCalendarFreeBusy (type)
  ```
  export interface GoogleCalendarFreeBusy {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 48): Unused export: GoogleCalendarGroup (type)
  ```
  export interface GoogleCalendarGroup {
  ```
  💡 Remove if unused or document why kept

### `src/types/shapeFieldMetadata.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 10): Unused export: ShapeFieldMetadata (type)
  ```
  export interface ShapeFieldMetadata {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 29): Unused export: ShapeLayoutConfig (type)
  ```
  export interface ShapeLayoutConfig {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 49): Unused export: ComposedFieldConfig (type)
  ```
  export interface ComposedFieldConfig {
  ```
  💡 Remove if unused or document why kept

### `src/utils/booking/partShapeTimeSlotMapping.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 54): Unused export: mapPartShapeToCategory (function)
  ```
  export function mapPartShapeToCategory(partShapeName: string | null | undefined): TimeSlotCategoryKey | null {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 110): Unused export: isValidTimeSlotCategory (function)
  ```
  export function isValidTimeSlotCategory(key: string): key is TimeSlotCategoryKey {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 18): Unused export: TimeSlotCategoryKey (type)
  ```
  export type TimeSlotCategoryKey =
  ```
  💡 Remove if unused or document why kept

### `src/utils/booking/timeSlotMatching.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 178): Unused export: findAppointmentSlotByOrderIndex (function)
  ```
  export function findAppointmentSlotByOrderIndex(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 198): Unused export: findMatchingAppointmentSlot (function)
  ```
  export function findMatchingAppointmentSlot(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 242): Unused export: matchLoadedTimeSlotsToAppointmentSlots (function)
  ```
  export function matchLoadedTimeSlotsToAppointmentSlots(
  ```
  💡 Remove if unused or document why kept

### `src/utils/optimistic/listByIdOptimistic.ts` [P1] (score: 9)

- **WARNING** [unused-export] (line 13): Unused export: optimisticUpsertById (function)
  ```
  export function optimisticUpsertById<TItem extends EntityWithStringId>(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 25): Unused export: optimisticRemoveById (function)
  ```
  export function optimisticRemoveById<TItem extends EntityWithStringId>(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 1): Unused export: EntityWithStringId (type)
  ```
  export type EntityWithStringId = { id: string }
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useEntityCardExpansion.ts` [P1] (score: 7)

- **WARNING** [unused-export] (line 12): Unused export: UseEntityCardExpansionOptions (type)
  ```
  export interface UseEntityCardExpansionOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 16): Unused export: UseEntityCardExpansionReturn (type)
  ```
  export interface UseEntityCardExpansionReturn {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 48): Unused function: handleExpansionChange
  ```
  function handleExpansionChange(event: { value: boolean }): void {
  ```
  💡 Remove if unused or document why kept

### `src/composables/componentEntity/useComponentEntityDomain.ts` [P1] (score: 7)

- **WARNING** [unused-export] (line 10): Unused export: UseComponentEntityDomainParams (type)
  ```
  export type UseComponentEntityDomainParams<GE extends GlobalEntityKey> = {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 16): Unused export: UseComponentEntityDomainReturn (type)
  ```
  export type UseComponentEntityDomainReturn<GE extends GlobalEntityKey> = {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 161): Unused function: newValues
  ```
  const newValues = (() => {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/annotationAssignments/useAnnotationAssignmentsOrchestration.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 18): Unused export: UseAnnotationAssignmentsOrchestrationOptions (type)
  ```
  export interface UseAnnotationAssignmentsOrchestrationOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 33): Unused export: UseAnnotationAssignmentsOrchestrationReturn (type)
  ```
  export type UseAnnotationAssignmentsOrchestrationReturn = {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useAnnotationTypeFiltering.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 20): Unused export: useAnnotationTypeFiltering (function)
  ```
  export function useAnnotationTypeFiltering(): UseAnnotationTypeFilteringReturn {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 10): Unused export: UseAnnotationTypeFilteringReturn (type)
  ```
  export interface UseAnnotationTypeFilteringReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useConditionalFieldVisibility.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 27): Unused export: UseConditionalFieldVisibilityOptions (type)
  ```
  export interface UseConditionalFieldVisibilityOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 34): Unused export: UseConditionalFieldVisibilityReturn (type)
  ```
  export interface UseConditionalFieldVisibilityReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useEntityCardActions.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 29): Unused export: UseEntityCardActionsOptions (type)
  ```
  export interface UseEntityCardActionsOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 69): Unused export: UseEntityCardActionsReturn (type)
  ```
  export interface UseEntityCardActionsReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useEntityCardComputed.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 20): Unused export: UseEntityCardComputedParams (type)
  ```
  export interface UseEntityCardComputedParams<GE extends GlobalEntityKey> {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 30): Unused export: UseEntityCardComputedReturn (type)
  ```
  export interface UseEntityCardComputedReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useEntityCardFieldConfiguration.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 18): Unused export: UseEntityCardFieldConfigurationParams (type)
  ```
  export interface UseEntityCardFieldConfigurationParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 48): Unused export: UseEntityCardFieldConfigurationReturn (type)
  ```
  export interface UseEntityCardFieldConfigurationReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useEntityCardMetadata.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 18): Unused export: UseEntityCardMetadataParams (type)
  ```
  export interface UseEntityCardMetadataParams<GE extends GlobalEntityKey> {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 38): Unused export: UseEntityCardMetadataReturn (type)
  ```
  export interface UseEntityCardMetadataReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useEntityCardSaveState.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 16): Unused export: UseEntityCardSaveStateOptions (type)
  ```
  export interface UseEntityCardSaveStateOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 28): Unused export: UseEntityCardSaveStateReturn (type)
  ```
  export interface UseEntityCardSaveStateReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useEntityCardStoreSync.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 17): Unused export: UseEntityCardStoreSyncOptions (type)
  ```
  export interface UseEntityCardStoreSyncOptions<GE extends GlobalEntityKey> {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 52): Unused export: UseEntityCardStoreSyncReturn (type)
  ```
  export interface UseEntityCardStoreSyncReturn<GE extends GlobalEntityKey> {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useEntityDragHandlers.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 23): Unused export: UseEntityDragHandlersParams (type)
  ```
  export interface UseEntityDragHandlersParams<EntityKey extends GlobalEntityKey> {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 33): Unused export: UseEntityDragHandlersReturn (type)
  ```
  export interface UseEntityDragHandlersReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useEntityList.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 23): Unused export: UseEntityListOptions (type)
  ```
  export interface UseEntityListOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 70): Unused export: UseEntityListReturn (type)
  ```
  export interface UseEntityListReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useEntityStatus.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 25): Unused export: UseEntityStatusOptions (type)
  ```
  export interface UseEntityStatusOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 40): Unused export: UseEntityStatusReturn (type)
  ```
  export interface UseEntityStatusReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useEntityTabState.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 19): Unused export: UseEntityTabStateOptions (type)
  ```
  export interface UseEntityTabStateOptions<EntityKey extends GlobalEntityKey> {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 38): Unused export: UseEntityTabStateReturn (type)
  ```
  export interface UseEntityTabStateReturn {}
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useFieldComponent.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 25): Unused export: UseFieldComponentOptions (type)
  ```
  export interface UseFieldComponentOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 58): Unused export: UseFieldComponentReturn (type)
  ```
  export interface UseFieldComponentReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useFieldContextManager.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 16): Unused export: UseFieldContextManagerOptions (type)
  ```
  export interface UseFieldContextManagerOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 32): Unused export: UseFieldContextManagerReturn (type)
  ```
  export interface UseFieldContextManagerReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useFieldLocation.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 28): Unused export: UseFieldLocationOptions (type)
  ```
  export interface UseFieldLocationOptions<GE extends GlobalEntityKey> {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 49): Unused export: UseFieldLocationReturn (type)
  ```
  export interface UseFieldLocationReturn<GE extends GlobalEntityKey> {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useFieldRendererComponent.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 12): Unused export: UseFieldRendererComponentOptions (type)
  ```
  export interface UseFieldRendererComponentOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 29): Unused export: UseFieldRendererComponentReturn (type)
  ```
  export interface UseFieldRendererComponentReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useFormElementPatching.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 26): Unused export: UseFormElementPatchingOptions (type)
  ```
  export interface UseFormElementPatchingOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 52): Unused export: UseFormElementPatchingReturn (type)
  ```
  export interface UseFormElementPatchingReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useIconPickerState.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 19): Unused export: UseIconPickerStateOptions (type)
  ```
  export interface UseIconPickerStateOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 34): Unused export: UseIconPickerStateReturn (type)
  ```
  export interface UseIconPickerStateReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useInstanceBulkEdit.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 23): Unused export: UseInstanceBulkEditOptions (type)
  ```
  export interface UseInstanceBulkEditOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 33): Unused export: UseInstanceBulkEditReturn (type)
  ```
  export interface UseInstanceBulkEditReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useInstanceDragAndDrop.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 16): Unused export: UseInstanceDragAndDropOptions (type)
  ```
  export interface UseInstanceDragAndDropOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 21): Unused export: UseInstanceDragAndDropReturn (type)
  ```
  export interface UseInstanceDragAndDropReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useInstanceFiltering.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 22): Unused export: UseInstanceFilteringOptions (type)
  ```
  export interface UseInstanceFilteringOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 26): Unused export: UseInstanceFilteringReturn (type)
  ```
  export interface UseInstanceFilteringReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useInstanceGrouping.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 23): Unused export: UseInstanceGroupingOptions (type)
  ```
  export interface UseInstanceGroupingOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 33): Unused export: UseInstanceGroupingReturn (type)
  ```
  export interface UseInstanceGroupingReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useInstanceOverrideState.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 13): Unused export: UseInstanceOverrideStateOptions (type)
  ```
  export interface UseInstanceOverrideStateOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 18): Unused export: UseInstanceOverrideStateReturn (type)
  ```
  export interface UseInstanceOverrideStateReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useInstanceShape.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 12): Unused export: UseInstanceShapeOptions (type)
  ```
  export interface UseInstanceShapeOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 17): Unused export: UseInstanceShapeReturn (type)
  ```
  export interface UseInstanceShapeReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useInstanceTabHandlers.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 9): Unused export: UseInstanceTabHandlersOptions (type)
  ```
  export interface UseInstanceTabHandlersOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 13): Unused export: UseInstanceTabHandlersReturn (type)
  ```
  export interface UseInstanceTabHandlersReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useMetadataCache.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 207): Unused export: resetMetadataCache (function)
  ```
  export function resetMetadataCache(): void {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 43): Unused function: fetchAllAdminMetadata
  ```
  async function fetchAllAdminMetadata(): Promise<MetadataCache> {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 155): Unused function: invalidateMetadataCache
  ```
  function invalidateMetadataCache(): void {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 164): Unused function: getMetadataCache
  ```
  function getMetadataCache(): MetadataCache | null {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useMetadataFieldOrdering.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 15): Unused export: UseMetadataFieldOrderingOptions (type)
  ```
  export interface UseMetadataFieldOrderingOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 21): Unused export: UseMetadataFieldOrderingReturn (type)
  ```
  export interface UseMetadataFieldOrderingReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useMetadataFieldUpdates.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 12): Unused export: UseMetadataFieldUpdatesOptions (type)
  ```
  export interface UseMetadataFieldUpdatesOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 20): Unused export: UseMetadataFieldUpdatesReturn (type)
  ```
  export interface UseMetadataFieldUpdatesReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useMetadataModalHandlers.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 20): Unused export: useMetadataModalHandlers (function)
  ```
  export function useMetadataModalHandlers(): UseMetadataModalHandlersReturn {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 9): Unused export: UseMetadataModalHandlersReturn (type)
  ```
  export interface UseMetadataModalHandlersReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/usePartInstanceBulkEdit.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 35): Unused export: UsePartInstanceBulkEditOptions (type)
  ```
  export interface UsePartInstanceBulkEditOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 45): Unused export: UsePartInstanceBulkEditReturn (type)
  ```
  export interface UsePartInstanceBulkEditReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/usePartInstanceExpansion.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 9): Unused export: UsePartInstanceExpansionOptions (type)
  ```
  export interface UsePartInstanceExpansionOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 13): Unused export: UsePartInstanceExpansionReturn (type)
  ```
  export interface UsePartInstanceExpansionReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useSelectConfig.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 29): Unused export: UseSelectConfigOptions (type)
  ```
  export interface UseSelectConfigOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 39): Unused export: UseSelectConfigReturn (type)
  ```
  export interface UseSelectConfigReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useSelectDomTargets.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 24): Unused export: UseSelectDomTargetsOptions (type)
  ```
  export interface UseSelectDomTargetsOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 41): Unused export: UseSelectDomTargetsReturn (type)
  ```
  export interface UseSelectDomTargetsReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useSelectFieldValue.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 28): Unused export: UseSelectFieldValueOptions (type)
  ```
  export interface UseSelectFieldValueOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 68): Unused export: UseSelectFieldValueReturn (type)
  ```
  export interface UseSelectFieldValueReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useSelectHandlers.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 27): Unused export: UseSelectHandlersOptions (type)
  ```
  export interface UseSelectHandlersOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 67): Unused export: UseSelectHandlersReturn (type)
  ```
  export interface UseSelectHandlersReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useSelectLabelResolution.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 17): Unused export: UseSelectLabelResolutionOptions (type)
  ```
  export interface UseSelectLabelResolutionOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 29): Unused export: UseSelectLabelResolutionReturn (type)
  ```
  export interface UseSelectLabelResolutionReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useShapeDeletion.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 19): Unused export: useShapeDeletion (function)
  ```
  export function useShapeDeletion(): UseShapeDeletionReturn {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 7): Unused export: UseShapeDeletionReturn (type)
  ```
  export interface UseShapeDeletionReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useShapeEditModal.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 10): Unused export: UseShapeEditModalOptions (type)
  ```
  export interface UseShapeEditModalOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 14): Unused export: UseShapeEditModalReturn (type)
  ```
  export interface UseShapeEditModalReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useStatusButtonFields.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 14): Unused export: UseStatusButtonFieldsOptions (type)
  ```
  export interface UseStatusButtonFieldsOptions<GE extends GlobalEntityKey> {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 19): Unused export: UseStatusButtonFieldsReturn (type)
  ```
  export interface UseStatusButtonFieldsReturn<GE extends GlobalEntityKey> {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useStatusButtonToggle.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 21): Unused export: UseStatusButtonToggleOptions (type)
  ```
  export interface UseStatusButtonToggleOptions<GE extends GlobalEntityKey> {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 33): Unused export: UseStatusButtonToggleReturn (type)
  ```
  export interface UseStatusButtonToggleReturn<GE extends GlobalEntityKey> {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useStatusButtonValue.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 17): Unused export: getStatusButtonBooleanValue (function)
  ```
  export function getStatusButtonBooleanValue<GE extends GlobalEntityKey>(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 40): Unused export: useStatusButtonValue (const)
  ```
  export const useStatusButtonValue = {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useTabNavigation.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 9): Unused export: UseTabNavigationOptions (type)
  ```
  export interface UseTabNavigationOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 13): Unused export: UseTabNavigationReturn (type)
  ```
  export interface UseTabNavigationReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/dev/usePanelPosition.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 16): Unused export: UsePanelPositionOptions (type)
  ```
  export interface UsePanelPositionOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 38): Unused export: UsePanelPositionReturn (type)
  ```
  export interface UsePanelPositionReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/selectionCard/useSelectionCardComponent.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 16): Unused export: UseSelectionCardComponentParams (type)
  ```
  export interface UseSelectionCardComponentParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 26): Unused export: UseSelectionCardComponentReturn (type)
  ```
  export interface UseSelectionCardComponentReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/selectionCard/useSelectionCardConfig.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 17): Unused export: UseSelectionCardConfigParams (type)
  ```
  export interface UseSelectionCardConfigParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 24): Unused export: UseSelectionCardConfigReturn (type)
  ```
  export interface UseSelectionCardConfigReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/selectionCard/useSelectionCardGroupState.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 15): Unused export: UseSelectionCardGroupStateParams (type)
  ```
  export interface UseSelectionCardGroupStateParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 25): Unused export: UseSelectionCardGroupStateReturn (type)
  ```
  export interface UseSelectionCardGroupStateReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/selectionCard/useSelectionCardHandlers.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 16): Unused export: UseSelectionCardHandlersParams (type)
  ```
  export interface UseSelectionCardHandlersParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 34): Unused export: UseSelectionCardHandlersReturn (type)
  ```
  export interface UseSelectionCardHandlersReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/selectionCard/useSelectionCardState.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 22): Unused export: UseSelectionCardStateParams (type)
  ```
  export interface UseSelectionCardStateParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 32): Unused export: UseSelectionCardStateReturn (type)
  ```
  export interface UseSelectionCardStateReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/selectionCard/useSelectionCardStyles.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 20): Unused export: UseSelectionCardStylesParams (type)
  ```
  export interface UseSelectionCardStylesParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 28): Unused export: UseSelectionCardStylesReturn (type)
  ```
  export interface UseSelectionCardStylesReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useAppointmentDataCollection.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 48): Unused export: UseAppointmentDataCollectionParams (type)
  ```
  export interface UseAppointmentDataCollectionParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 71): Unused export: UseAppointmentDataCollectionReturn (type)
  ```
  export interface UseAppointmentDataCollectionReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useAppointmentDropdown.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 13): Unused export: UseAppointmentDropdownOptions (type)
  ```
  export interface UseAppointmentDropdownOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 19): Unused export: UseAppointmentDropdownReturn (type)
  ```
  export interface UseAppointmentDropdownReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useAppointmentDuration.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 16): Unused export: UseAppointmentDurationParams (type)
  ```
  export interface UseAppointmentDurationParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 26): Unused export: UseAppointmentDurationReturn (type)
  ```
  export interface UseAppointmentDurationReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useAppointmentSlots.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 33): Unused export: UseAppointmentSlotsParams (type)
  ```
  export interface UseAppointmentSlotsParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 46): Unused export: UseAppointmentSlotsReturn (type)
  ```
  export interface UseAppointmentSlotsReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useAvailabilityDefaults.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 25): Unused export: UseAvailabilityDefaultsOptions (type)
  ```
  export interface UseAvailabilityDefaultsOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 45): Unused export: UseAvailabilityDefaultsReturn (type)
  ```
  export interface UseAvailabilityDefaultsReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useAvailabilityEmptyState.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 14): Unused export: UseAvailabilityEmptyStateParams (type)
  ```
  export interface UseAvailabilityEmptyStateParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 34): Unused export: UseAvailabilityEmptyStateReturn (type)
  ```
  export interface UseAvailabilityEmptyStateReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useAvailabilitySlotColor.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 14): Unused export: UseAvailabilitySlotColorParams (type)
  ```
  export interface UseAvailabilitySlotColorParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 24): Unused export: UseAvailabilitySlotColorReturn (type)
  ```
  export interface UseAvailabilitySlotColorReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useAvailabilityStepData.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 25): Unused export: UseAvailabilityStepDataParams (type)
  ```
  export interface UseAvailabilityStepDataParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 34): Unused export: UseAvailabilityStepDataReturn (type)
  ```
  export interface UseAvailabilityStepDataReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useAvailabilityStepHandlers.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 16): Unused export: UseAvailabilityStepHandlersParams (type)
  ```
  export interface UseAvailabilityStepHandlersParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 66): Unused export: UseAvailabilityStepHandlersReturn (type)
  ```
  export interface UseAvailabilityStepHandlersReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useAvailabilityUI.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 20): Unused export: UseAvailabilityUIParams (type)
  ```
  export interface UseAvailabilityUIParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 29): Unused export: UseAvailabilityUIReturn (type)
  ```
  export interface UseAvailabilityUIReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useAvailabilityValidation.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 18): Unused export: UseAvailabilityValidationParams (type)
  ```
  export interface UseAvailabilityValidationParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 26): Unused export: UseAvailabilityValidationReturn (type)
  ```
  export type UseAvailabilityValidationReturn = UseStepValidationReturn
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useBookingWizardStepValidators.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 6): Unused export: BookingWizardStepValidators (type)
  ```
  export interface BookingWizardStepValidators {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 14): Unused export: UseBookingWizardStepValidatorsOptions (type)
  ```
  export interface UseBookingWizardStepValidatorsOptions {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useBusyTimes.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 17): Unused export: UseBusyTimesParams (type)
  ```
  export interface UseBusyTimesParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 32): Unused export: UseBusyTimesReturn (type)
  ```
  export interface UseBusyTimesReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useCascadeInstances.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 21): Unused export: UseCascadeInstancesOptions (type)
  ```
  export interface UseCascadeInstancesOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 44): Unused export: UseCascadeInstancesReturn (type)
  ```
  export interface UseCascadeInstancesReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useConfirmationStepData.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 55): Unused export: UseConfirmationStepDataParams (type)
  ```
  export interface UseConfirmationStepDataParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 69): Unused export: UseConfirmationStepDataReturn (type)
  ```
  export interface UseConfirmationStepDataReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useContactsStepData.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 29): Unused export: UseContactsStepDataOptions (type)
  ```
  export interface UseContactsStepDataOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 39): Unused export: UseContactsStepDataReturn (type)
  ```
  export interface UseContactsStepDataReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useContactsValidation.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 19): Unused export: UseContactsValidationParams (type)
  ```
  export interface UseContactsValidationParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 33): Unused export: UseContactsValidationReturn (type)
  ```
  export type UseContactsValidationReturn = UseStepValidationReturn
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useDependentInstances.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 25): Unused export: UseDependentInstancesOptions (type)
  ```
  export interface UseDependentInstancesOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 41): Unused export: UseDependentInstancesReturn (type)
  ```
  export interface UseDependentInstancesReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useDynamicGridConfig.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 13): Unused export: UseDynamicGridConfigOptions (type)
  ```
  export interface UseDynamicGridConfigOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 25): Unused export: UseDynamicGridConfigReturn (type)
  ```
  export interface UseDynamicGridConfigReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useElementDimensions.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 19): Unused export: UseElementDimensionsOptions (type)
  ```
  export interface UseElementDimensionsOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 31): Unused export: UseElementDimensionsReturn (type)
  ```
  export interface UseElementDimensionsReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useInstanceComponents.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 26): Unused export: UseInstanceComponentsOptions (type)
  ```
  export interface UseInstanceComponentsOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 41): Unused export: UseInstanceComponentsReturn (type)
  ```
  export interface UseInstanceComponentsReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useInstanceDescriptions.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 24): Unused export: UseInstanceDescriptionsOptions (type)
  ```
  export interface UseInstanceDescriptionsOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 39): Unused export: UseInstanceDescriptionsReturn (type)
  ```
  export interface UseInstanceDescriptionsReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useInstanceDisplay.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 24): Unused export: UseInstanceDisplayOptions (type)
  ```
  export interface UseInstanceDisplayOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 41): Unused export: UseInstanceDisplayReturn (type)
  ```
  export interface UseInstanceDisplayReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useInstanceSelectionConfig.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 26): Unused export: UseInstanceSelectionConfigOptions (type)
  ```
  export interface UseInstanceSelectionConfigOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 49): Unused export: UseInstanceSelectionConfigReturn (type)
  ```
  export interface UseInstanceSelectionConfigReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useInstanceSelectionState.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 27): Unused export: GenericWizardInstance (type)
  ```
  export interface GenericWizardInstance {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 64): Unused export: UseInstanceSelectionStateReturn (type)
  ```
  export interface UseInstanceSelectionStateReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useOptionTypeBlockSelection.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 16): Unused export: UseOptionTypeBlockSelectionParams (type)
  ```
  export interface UseOptionTypeBlockSelectionParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 24): Unused export: UseOptionTypeBlockSelectionReturn (type)
  ```
  export interface UseOptionTypeBlockSelectionReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/usePerspectiveMapping.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 15): Unused export: UsePerspectiveMappingParams (type)
  ```
  export interface UsePerspectiveMappingParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 25): Unused export: UsePerspectiveMappingReturn (type)
  ```
  export interface UsePerspectiveMappingReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/usePropertyFormWatchers.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 18): Unused export: UsePropertyFormWatchersParams (type)
  ```
  export interface UsePropertyFormWatchersParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 28): Unused export: UsePropertyFormWatchersReturn (type)
  ```
  export interface UsePropertyFormWatchersReturn {}
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/usePropertyTypeBlockConfig.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 17): Unused export: UsePropertyTypeBlockConfigParams (type)
  ```
  export interface UsePropertyTypeBlockConfigParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 54): Unused export: UsePropertyTypeBlockConfigReturn (type)
  ```
  export interface UsePropertyTypeBlockConfigReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/usePropertyTypeBlockSelection.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 16): Unused export: UsePropertyTypeBlockSelectionParams (type)
  ```
  export interface UsePropertyTypeBlockSelectionParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 25): Unused export: UsePropertyTypeBlockSelectionReturn (type)
  ```
  export interface UsePropertyTypeBlockSelectionReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/usePropertyValidation.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 29): Unused export: UsePropertyValidationParams (type)
  ```
  export interface UsePropertyValidationParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 45): Unused export: UsePropertyValidationReturn (type)
  ```
  export type UsePropertyValidationReturn = UseStepValidationReturn
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useResponsiveGrid.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 20): Unused export: UseResponsiveGridOptions (type)
  ```
  export interface UseResponsiveGridOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 67): Unused export: UseResponsiveGridReturn (type)
  ```
  export interface UseResponsiveGridReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useSelectionCardGroupConfig.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 22): Unused export: UseSelectionCardGroupConfigParams (type)
  ```
  export interface UseSelectionCardGroupConfigParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 29): Unused export: UseSelectionCardGroupConfigReturn (type)
  ```
  export interface UseSelectionCardGroupConfigReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useStepValidation.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 15): Unused export: CustomValidator (type)
  ```
  export type CustomValidator = () => true | string
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 20): Unused export: UseStepValidationParams (type)
  ```
  export interface UseStepValidationParams {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useTimeSlotDurations.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 16): Unused export: UseTimeSlotDurationsParams (type)
  ```
  export interface UseTimeSlotDurationsParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 31): Unused export: UseTimeSlotDurationsReturn (type)
  ```
  export interface UseTimeSlotDurationsReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useWizardAppointmentManagement.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 18): Unused export: UseWizardAppointmentManagementOptions (type)
  ```
  export interface UseWizardAppointmentManagementOptions extends WizardStepDataAndValidationRefs {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 34): Unused export: UseWizardAppointmentManagementReturn (type)
  ```
  export interface UseWizardAppointmentManagementReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useWizardDevMode.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 13): Unused export: UseWizardDevModeOptions (type)
  ```
  export interface UseWizardDevModeOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 27): Unused export: UseWizardDevModeReturn (type)
  ```
  export interface UseWizardDevModeReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useWizardDisplay.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 25): Unused export: UseWizardDisplayParams (type)
  ```
  export interface UseWizardDisplayParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 34): Unused export: UseWizardDisplayReturn (type)
  ```
  export interface UseWizardDisplayReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useWizardFilteredOptions.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 94): Unused export: UseWizardFilteredOptionsParams (type)
  ```
  export type UseWizardFilteredOptionsParams = {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 105): Unused export: UseWizardFilteredOptionsReturn (type)
  ```
  export type UseWizardFilteredOptionsReturn = WizardComputedProperties
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useWizardNavigation.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 23): Unused export: UseWizardNavigationParams (type)
  ```
  export interface UseWizardNavigationParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 32): Unused export: UseWizardNavigationReturn (type)
  ```
  export interface UseWizardNavigationReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useWizardSubmission.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 15): Unused export: UseWizardSubmissionParams (type)
  ```
  export interface UseWizardSubmissionParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 29): Unused export: UseWizardSubmissionReturn (type)
  ```
  export interface UseWizardSubmissionReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useWizardValidation.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 18): Unused export: UseWizardValidationParams (type)
  ```
  export interface UseWizardValidationParams {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 25): Unused export: UseWizardValidationReturn (type)
  ```
  export interface UseWizardValidationReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useWizardValidationErrors.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 14): Unused export: UseWizardValidationErrorsOptions (type)
  ```
  export interface UseWizardValidationErrorsOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 27): Unused export: UseWizardValidationErrorsReturn (type)
  ```
  export interface UseWizardValidationErrorsReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/businessDataCollections/types.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 43): Unused export: BusinessDataCollectionUpdater (type)
  ```
  export type BusinessDataCollectionUpdater<CollectionItem> = (
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 51): Unused export: BusinessDataCollectionEndpoints (type)
  ```
  export type BusinessDataCollectionEndpoints = {
  ```
  💡 Remove if unused or document why kept

### `src/composables/entityCrud/useSharedMutationHandlers.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 58): Unused export: createRestorePreviousDataHandler (function)
  ```
  export function createRestorePreviousDataHandler(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 83): Unused export: MutationContextWithPreviousData (type)
  ```
  export interface MutationContextWithPreviousData {
  ```
  💡 Remove if unused or document why kept

### `src/composables/globalDataCollections/types.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 21): Unused export: GlobalDataCollectionUpdater (type)
  ```
  export type GlobalDataCollectionUpdater<CollectionItem> = (
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 26): Unused export: GlobalDataCollectionEndpoints (type)
  ```
  export type GlobalDataCollectionEndpoints = {
  ```
  💡 Remove if unused or document why kept

### `src/composables/useComponentDistribution.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 23): Unused export: UseComponentDistributionOptions (type)
  ```
  export interface UseComponentDistributionOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 39): Unused export: UseComponentDistributionReturn (type)
  ```
  export interface UseComponentDistributionReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/useEntityForm.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 20): Unused export: UseEntityFormOptions (type)
  ```
  export interface UseEntityFormOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 30): Unused export: UseEntityFormReturn (type)
  ```
  export interface UseEntityFormReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/useLayoutLoading.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 27): Unused export: UseLayoutLoadingOptions (type)
  ```
  export interface UseLayoutLoadingOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 37): Unused export: UseLayoutLoadingReturn (type)
  ```
  export interface UseLayoutLoadingReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/usePartInstanceData.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 23): Unused export: UsePartInstanceDataOptions (type)
  ```
  export interface UsePartInstanceDataOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 30): Unused export: UsePartInstanceDataReturn (type)
  ```
  export interface UsePartInstanceDataReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/useSelectOptions.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 40): Unused export: UseSelectOptionsOptions (type)
  ```
  export interface UseSelectOptionsOptions {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 53): Unused export: UseSelectOptionsReturn (type)
  ```
  export interface UseSelectOptionsReturn {
  ```
  💡 Remove if unused or document why kept

### `src/configs/field/form/_archived/primitiveFieldConfig.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 18): Unused export: buildPrimitiveFieldType (function)
  ```
  export function buildPrimitiveFieldType(): PrimitiveFieldTypeMap {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 14): Unused export: PrimitiveFieldTypeMap (type)
  ```
  export type PrimitiveFieldTypeMap = {
  ```
  💡 Remove if unused or document why kept

### `src/configs/field/form/_archived/selectableFieldConfig.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 95): Unused export: buildSelectableFieldType (function)
  ```
  export function buildSelectableFieldType(): SelectableFieldTypeSuite {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 80): Unused export: SelectableFieldType (type)
  ```
  export type SelectableFieldType<
  ```
  💡 Remove if unused or document why kept

### `src/types/component.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 24): Unused export: ComponentConfig (type)
  ```
  export interface ComponentConfig {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 35): Unused export: FetchedInstanceComponent (type)
  ```
  export interface FetchedInstanceComponent {
  ```
  💡 Remove if unused or document why kept

### `src/types/entities.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 57): Unused export: PartInstanceEntity (type)
  ```
  export interface PartInstanceEntity extends BaseGlobalEntity<"partInstance"> {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 85): Unused export: GlobalEntityMap (type)
  ```
  export type GlobalEntityMap = {
  ```
  💡 Remove if unused or document why kept

### `src/types/property.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 60): Unused export: PropertyVersionType (type)
  ```
  export interface PropertyVersionType {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 79): Unused export: PropertyTypesRequest (type)
  ```
  export interface PropertyTypesRequest {
  ```
  💡 Remove if unused or document why kept

### `src/types/wizard.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 22): Unused export: WizardState (type)
  ```
  export interface WizardState {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 41): Unused export: WizardSelectionMethods (type)
  ```
  export interface WizardSelectionMethods {
  ```
  💡 Remove if unused or document why kept

### `src/utils/api.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 263): Unused export: getPropertyTypesEndpoint (function)
  ```
  export function getPropertyTypesEndpoint(propertyVersionId: string): string {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 267): Unused export: getPropertyTypeByIdEndpoint (function)
  ```
  export function getPropertyTypeByIdEndpoint(propertyVersionId: string, typeId: string): string {
  ```
  💡 Remove if unused or document why kept

### `src/utils/appointmentFieldFormatters.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 119): Unused export: APPOINTMENT_FIELD_FORMATTERS (const)
  ```
  export const APPOINTMENT_FIELD_FORMATTERS: Record<string, FieldFormatter> = {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 19): Unused export: FieldFormatter (type)
  ```
  export type FieldFormatter = (
  ```
  💡 Remove if unused or document why kept

### `src/utils/booking/confirmationStepData.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 27): Unused export: BlockInstanceFeeResult (type)
  ```
  export interface BlockInstanceFeeResult {
  ```
  💡 Remove if unused or document why kept

- **INFO** [todo-marker] (line 252): TODO/FIXME marker about unused code
  ```
  const couponDiscount = 0 // TODO: Remove hardcoded value when coupon system is implemented
  ```
  💡 Review and clean up unused code

- **INFO** [todo-marker] (line 254): TODO/FIXME marker about unused code
  ```
  const deliveryCharges = 5.0 // TODO: Remove hardcoded value when business settings integration is im...
  ```
  💡 Review and clean up unused code

- **INFO** [todo-marker] (line 255): TODO/FIXME marker about unused code
  ```
  const deliveryFree = true // TODO: Remove hardcoded value when business settings integration is impl...
  ```
  💡 Review and clean up unused code

### `src/utils/datetime.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 33): Unused export: rfc3339ToDateOnly (function)
  ```
  export function rfc3339ToDateOnly(rfc3339: RFC3339DateTime): ISO8601Date {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 55): Unused export: dateOnlyToRfc3339 (function)
  ```
  export function dateOnlyToRfc3339(date: ISO8601Date): RFC3339DateTime {
  ```
  💡 Remove if unused or document why kept

### `src/utils/forms/fieldSectionCategorization.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 24): Unused export: CategorizedFields (type)
  ```
  export interface CategorizedFields {
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 51): Unused export: CategorizeFieldsOptions (type)
  ```
  export interface CategorizeFieldsOptions {
  ```
  💡 Remove if unused or document why kept

### `src/utils/logger.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 15): Unused export: LogLevel (type)
  ```
  export type LogLevel = 'debug' | 'info' | 'warn' | 'error' | 'silent'
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 17): Unused export: AppLogger (type)
  ```
  export type AppLogger = {
  ```
  💡 Remove if unused or document why kept

### `src/utils/optimistic/annotationAssignmentsOptimistic.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 67): Unused export: deriveBlockInstanceDescriptionFromAnnotations (function)
  ```
  export function deriveBlockInstanceDescriptionFromAnnotations(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 31): Unused export: AnnotationAssignmentPatch (type)
  ```
  export type AnnotationAssignmentPatch = Partial<Pick<AnnotationAssignmentLike, 'orderIndex' | 'isDefault' | 'userTypeBlockBlockInstanceId'>>
  ```
  💡 Remove if unused or document why kept

### `src/utils/transformers/annotationTransformers.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 120): Unused export: transformAnnotationsWithMetadata (function)
  ```
  export function transformAnnotationsWithMetadata(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 224): Unused export: getThroughAttributes (function)
  ```
  export function getThroughAttributes(annotation: Record<string, unknown>): {
  ```
  💡 Remove if unused or document why kept

### `src/utils/transformers/composePropertyValue.ts` [P1] (score: 6)

- **WARNING** [unused-export] (line 37): Unused export: composeProperty (function)
  ```
  export function composeProperty<T extends string | number | boolean | unknown[]>(
  ```
  💡 Remove if unused or document why kept

- **WARNING** [unused-export] (line 15): Unused export: ComposablePropertyValue (type)
  ```
  export type ComposablePropertyValue = string | number | boolean | unknown[]
  ```
  💡 Remove if unused or document why kept

### `src/composables/useLocalTime.ts` [P1] (score: 5)

- **WARNING** [unused-export] (line 435): Unused export: UseLocalTimeReturn (type)
  ```
  export interface UseLocalTimeReturn {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 31): Unused function: rfc3339ToLocalTime
  ```
  function rfc3339ToLocalTime(rfc3339: RFC3339DateTime): Date {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 163): Unused function: rfc3339ToLocalTimeOfDay
  ```
  function rfc3339ToLocalTimeOfDay(rfc3339: RFC3339DateTime): string {
  ```
  💡 Remove if unused or document why kept

### `src/composables/formFields/useFormFieldsContext.ts` [P2] (score: 4)

- **WARNING** [unused-export] (line 25): Unused export: UseFormFieldsContextReturn (type)
  ```
  export type UseFormFieldsContextReturn = {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 287): Unused function: buildContext
  ```
  const buildContext = () => {
  ```
  💡 Remove if unused or document why kept

### `src/layouts/components/NavBarNotifications.vue` [P2] (score: 4)

- **INFO** [unused-function] (line 53): Unused function: removeNotification
  ```
  const removeNotification = (notificationId: number) => {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 60): Unused function: markRead
  ```
  const markRead = (notificationId: number[]) => {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 70): Unused function: markUnRead
  ```
  const markUnRead = (notificationId: number[]) => {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 80): Unused function: handleNotificationClick
  ```
  const handleNotificationClick = (notification: Notification) => {
  ```
  💡 Remove if unused or document why kept

### `src/plugins/1.router/additional-routes.ts` [P2] (score: 4)

- **WARNING** [unused-export] (line 6): Unused export: redirects (const)
  ```
  export const redirects: RouteRecordRaw[] = [
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 3): Unused function: emailRouteComponent
  ```
  const emailRouteComponent = () => import('@/pages/apps/email/index.vue')
  ```
  💡 Remove if unused or document why kept

### `src/components/admin/generic/fields/IconPicker.vue` [P2] (score: 3)

- **INFO** [unused-function] (line 161): Unused function: handleDialogUpdate
  ```
  const handleDialogUpdate = (value: boolean) => {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 171): Unused function: handleIconSelect
  ```
  const handleIconSelect = (icon: string) => {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 190): Unused function: handleClose
  ```
  const handleClose = () => {
  ```
  💡 Remove if unused or document why kept

### `src/components/admin/generic/fields/TextInput.vue` [P2] (score: 3)

- **INFO** [unused-function] (line 232): Unused function: handleFocus
  ```
  const handleFocus = () => handlers.value.handleFocus()
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 233): Unused function: handleBlur
  ```
  const handleBlur = () => handlers.value.handleBlur()
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 234): Unused function: handleEnterKey
  ```
  const handleEnterKey = (event: KeyboardEvent) => handlers.value.handleEnterKey(event)
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/annotationAssignments/useAnnotationAssignmentsMutations.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 31): Unused export: UseAnnotationAssignmentsMutationsOptions (type)
  ```
  export interface UseAnnotationAssignmentsMutationsOptions {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/tables/useAppointmentsTableModel.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 12): Unused export: AppointmentsTableModel (type)
  ```
  export interface AppointmentsTableModel extends CrudDataTableModel<
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/tables/useCrudDataTableModel.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 11): Unused export: CrudDataTableModelOptions (type)
  ```
  export interface CrudDataTableModelOptions<
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/tables/usePropertiesTableModel.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 7): Unused export: PropertiesTableModel (type)
  ```
  export interface PropertiesTableModel extends CrudDataTableModel<
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/tables/useUsersTableModel.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 7): Unused export: UsersTableModel (type)
  ```
  export interface UsersTableModel extends CrudDataTableModel<
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useAdminPrimitiveMetadataMutations.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 31): Unused export: useAdminPrimitiveMetadataMutations (function)
  ```
  export function useAdminPrimitiveMetadataMutations() {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useAdminRelationshipMetadataMutations.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 27): Unused export: useAdminRelationshipMetadataMutations (function)
  ```
  export function useAdminRelationshipMetadataMutations() {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useAnnotationSelect.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 54): Unused export: UseAnnotationSelectOptions (type)
  ```
  export interface UseAnnotationSelectOptions {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useAnnotationsFieldViewModel.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 13): Unused export: AnnotationsFieldViewModel (type)
  ```
  export interface AnnotationsFieldViewModel {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useAvailabilitySettings.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 45): Unused export: UseAvailabilitySettingsReturn (type)
  ```
  export interface UseAvailabilitySettingsReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useEntityDisplay.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 22): Unused export: UseEntityDisplayReturn (type)
  ```
  export interface UseEntityDisplayReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useEntityFiltering.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 17): Unused export: UseEntityFilteringReturn (type)
  ```
  export interface UseEntityFilteringReturn<EntityKey extends GlobalEntityKey> {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useExpansionState.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 18): Unused export: UseExpansionStateReturn (type)
  ```
  export interface UseExpansionStateReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useFieldInputHandlers.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 18): Unused export: UseFieldInputHandlersParams (type)
  ```
  export interface UseFieldInputHandlersParams {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useInstanceDeletion.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 7): Unused export: UseInstanceDeletionReturn (type)
  ```
  export interface UseInstanceDeletionReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useInstanceSaveHandlers.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 10): Unused export: UseInstanceSaveHandlersReturn (type)
  ```
  export interface UseInstanceSaveHandlersReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/usePartInstanceCollection.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 12): Unused export: PartInstanceCollectionModel (type)
  ```
  export interface PartInstanceCollectionModel {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/usePartInstanceDeletion.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 7): Unused export: UsePartInstanceDeletionReturn (type)
  ```
  export interface UsePartInstanceDeletionReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useSelectFiltering.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 31): Unused export: UseSelectFilteringOptions (type)
  ```
  export interface UseSelectFilteringOptions {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useSelectFormAssociation.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 20): Unused export: UseSelectFormAssociationOptions (type)
  ```
  export interface UseSelectFormAssociationOptions {
  ```
  💡 Remove if unused or document why kept

### `src/composables/admin/useShapeDisplayNames.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 16): Unused export: UseShapeDisplayNamesReturn (type)
  ```
  export interface UseShapeDisplayNamesReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useAvailabilityDevPanel.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 19): Unused export: UseAvailabilityDevPanelParams (type)
  ```
  export interface UseAvailabilityDevPanelParams {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useInstanceComponentsList.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 24): Unused export: UseInstanceComponentsListOptions (type)
  ```
  export interface UseInstanceComponentsListOptions {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useMockCalendarRefresh.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 14): Unused export: UseMockCalendarRefreshReturn (type)
  ```
  export interface UseMockCalendarRefreshReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/usePropertyFormState.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 20): Unused export: UsePropertyFormStateReturn (type)
  ```
  export interface UsePropertyFormStateReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useWizardStepContent.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 15): Unused export: UseWizardStepContentReturn (type)
  ```
  export interface UseWizardStepContentReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useWizardStepDataRefs.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 13): Unused export: UseWizardStepDataRefsReturn (type)
  ```
  export interface UseWizardStepDataRefsReturn extends WizardStepDataAndValidationRefs {}
  ```
  💡 Remove if unused or document why kept

### `src/composables/booking/useWizardStepSync.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 14): Unused export: UseWizardStepSyncParams (type)
  ```
  export interface UseWizardStepSyncParams<TStepData> {
  ```
  💡 Remove if unused or document why kept

### `src/composables/componentEntity/useComponentEntityActions.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 10): Unused export: UseComponentEntityActionsReturn (type)
  ```
  export type UseComponentEntityActionsReturn = {
  ```
  💡 Remove if unused or document why kept

### `src/composables/componentEntity/useComponentEntityQuery.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 6): Unused export: UseComponentEntityQueryReturn (type)
  ```
  export type UseComponentEntityQueryReturn = {
  ```
  💡 Remove if unused or document why kept

### `src/composables/dataCollections/useDataCollectionActions.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 18): Unused export: DataCollectionCrudConfig (type)
  ```
  export interface DataCollectionCrudConfig<
  ```
  💡 Remove if unused or document why kept

### `src/composables/entityCrud/useEntityCrudQuery.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 6): Unused export: UseEntityCrudQueryReturn (type)
  ```
  export type UseEntityCrudQueryReturn<GlobalEntityTypeKey extends GlobalEntityKey> = {
  ```
  💡 Remove if unused or document why kept

### `src/composables/fieldContext/useFieldContextActions.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 14): Unused export: UseFieldContextActionsReturn (type)
  ```
  export type UseFieldContextActionsReturn = {
  ```
  💡 Remove if unused or document why kept

### `src/composables/formFields/useFormFieldsStandardLayout.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 13): Unused export: UseFormFieldsStandardLayoutReturn (type)
  ```
  export type UseFormFieldsStandardLayoutReturn = {
  ```
  💡 Remove if unused or document why kept

### `src/composables/useAnnotationTypes.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 23): Unused export: UseAnnotationTypesReturn (type)
  ```
  export type UseAnnotationTypesReturn = {
  ```
  💡 Remove if unused or document why kept

### `src/composables/useFormValidation.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 29): Unused export: ValidationResult (type)
  ```
  export interface ValidationResult {
  ```
  💡 Remove if unused or document why kept

### `src/composables/useLoadingIndicator.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 16): Unused export: UseLoadingIndicatorReturn (type)
  ```
  export interface UseLoadingIndicatorReturn {
  ```
  💡 Remove if unused or document why kept

### `src/composables/useTimeFormatting.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 19): Unused export: UseTimeFormattingReturn (type)
  ```
  export interface UseTimeFormattingReturn {
  ```
  💡 Remove if unused or document why kept

### `src/configs/adminConfig.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 86): Unused export: buildAdminConfig (function)
  ```
  * NOTE: Internal function only - not exported as it's only used by buildAdminConfig
  ```
  💡 Remove if unused or document why kept

### `src/configs/field/form/fullFieldFormConfig.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 41): Unused export: buildAllPerEntityFieldConfig (function)
  ```
  export function buildAllPerEntityFieldConfig<GE extends GlobalEntityKey>(
  ```
  💡 Remove if unused or document why kept

### `src/configs/wizardSteps.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 17): Unused export: WizardStepConfig (type)
  ```
  export interface WizardStepConfig {
  ```
  💡 Remove if unused or document why kept

### `src/plugins/1.router/guards.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 9): Unused export: setupGuards (const)
  ```
  export const setupGuards = (router: _RouterTyped<RouteNamedMap & { [key: string]: unknown }>) => {
  ```
  💡 Remove if unused or document why kept

### `src/plugins/5.vuetify/theme.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 27): Unused export: inactiveColors (const)
  ```
  export const inactiveColors = {
  ```
  💡 Remove if unused or document why kept

### `src/shims/vue-i18n.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 17): Unused export: UseI18nOptions (type)
  ```
  export type UseI18nOptions = {
  ```
  💡 Remove if unused or document why kept

### `src/types/admin/AdminEntity.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 287): Unused export: AdminEntityMap (type)
  ```
  export type AdminEntityMap = {
  ```
  💡 Remove if unused or document why kept

### `src/types/annotations.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 63): Unused export: AnnotationMap (type)
  ```
  export type AnnotationMap = Record<GlobalAnnotationKey, AnnotationWithMetadata[]>
  ```
  💡 Remove if unused or document why kept

### `src/types/entity/formFields.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 100): Unused export: SelectableFormFieldType (type)
  ```
  export type SelectableFormFieldType<GE extends GlobalEntityKey = GlobalEntityKey> =
  ```
  💡 Remove if unused or document why kept

### `src/utils/admin/buildMetadataEntry.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 12): Unused export: BuildMetadataEntryOptions (type)
  ```
  export interface BuildMetadataEntryOptions {
  ```
  💡 Remove if unused or document why kept

### `src/utils/admin/entityDisplayText.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 33): Unused export: getEntityTypeName (function)
  ```
  export function getEntityTypeName(entityKey: GlobalEntityKey): string {
  ```
  💡 Remove if unused or document why kept

### `src/utils/autocomplete.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 4): Unused export: AutocompleteValue (type)
  ```
  export type AutocompleteValue = 'false' | 'off' | 'nope'
  ```
  💡 Remove if unused or document why kept

### `src/utils/booking/appointmentTimeCalculations.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 259): Unused export: calculateTotalDurationFromAppointmentSlots (function)
  ```
  export function calculateTotalDurationFromAppointmentSlots(appointmentSlots: AppointmentSlots): number {
  ```
  💡 Remove if unused or document why kept

### `src/utils/booking/slotGenerationValidation.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 19): Unused export: SlotGenerationParams (type)
  ```
  export interface SlotGenerationParams {
  ```
  💡 Remove if unused or document why kept

### `src/utils/collections/resolveByIds.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 4): Unused export: ResolveByIdsResult (type)
  ```
  export interface ResolveByIdsResult<CollectionItem extends IdentifiableById> {
  ```
  💡 Remove if unused or document why kept

### `src/utils/differentialScheduling.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 129): Unused export: calculateClientStartTimeFromInspector (function)
  ```
  export function calculateClientStartTimeFromInspector(inspectorStartTime: string, onSiteTotal: number): string {
  ```
  💡 Remove if unused or document why kept

### `src/utils/entities/entityFieldPatch.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 10): Unused export: createEntityFieldPatch (function)
  ```
  export function createEntityFieldPatch<
  ```
  💡 Remove if unused or document why kept

### `src/utils/forms/formElementPatching.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 15): Unused export: FormElementPatchingOptions (type)
  ```
  export type FormElementPatchingOptions = {
  ```
  💡 Remove if unused or document why kept

### `src/utils/forms/getFieldKeys.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 22): Unused export: GetFieldKeysOptions (type)
  ```
  export interface GetFieldKeysOptions<GE extends GlobalEntityKey> {
  ```
  💡 Remove if unused or document why kept

### `src/utils/transformers/componentAggregator.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 153): Unused export: composeProperties (function)
  ```
  export function composeProperties<GE extends GlobalEntityKey>(
  ```
  💡 Remove if unused or document why kept

### `src/utils/transformers/relationshipTransformers.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 276): Unused export: composePropertiesFromRelationships (function)
  ```
  export function composePropertiesFromRelationships<GE extends GlobalEntityKey>(
  ```
  💡 Remove if unused or document why kept

### `src/utils/wizardStateFieldConfig.ts` [P2] (score: 3)

- **WARNING** [unused-export] (line 38): Unused export: WizardFieldConfig (type)
  ```
  export interface WizardFieldConfig {
  ```
  💡 Remove if unused or document why kept

### `src/components/admin/generic/EntityFormContent.vue` [P2] (score: 2)

- **INFO** [unused-function] (line 150): Unused function: getNameFieldContext
  ```
  const getNameFieldContext = () => {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 154): Unused function: getActiveFieldContext
  ```
  const getActiveFieldContext = () => {
  ```
  💡 Remove if unused or document why kept

### `src/composables/entityCrud/useEntityCrud.ts` [P2] (score: 2)

- **INFO** [unused-function] (line 29): Unused function: isComposer
  ```
  function isComposer(entityId: GlobalEntityId): boolean {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 42): Unused function: updateWithComponentCheck
  ```
  async function updateWithComponentCheck(
  ```
  💡 Remove if unused or document why kept

### `src/layouts/components/NavSearchBar.vue` [P2] (score: 2)

- **INFO** [unused-function] (line 110): Unused function: fetchResults
  ```
  const fetchResults = async () => {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 125): Unused function: closeSearchBar
  ```
  const closeSearchBar = () => {
  ```
  💡 Remove if unused or document why kept

### `src/views/admin/tabs/BusinessControlsTab.vue` [P2] (score: 2)

- **INFO** [unused-function] (line 135): Unused function: ensureMaxWorkHours
  ```
  const ensureMaxWorkHours = (current: MaxWorkHours | undefined) => {
  ```
  💡 Remove if unused or document why kept

- **INFO** [unused-function] (line 322): Unused function: ensureRangeConstraints
  ```
  const ensureRangeConstraints = (current: RangeConstraints | undefined) => {
  ```
  💡 Remove if unused or document why kept

### `src/components/admin/BlockInstanceCreateModal.vue` [P2] (score: 1)

- **INFO** [unused-function] (line 191): Unused function: handleCreate
  ```
  async function handleCreate(): Promise<void> {
  ```
  💡 Remove if unused or document why kept

### `src/components/admin/MetadataEditModal.vue` [P2] (score: 1)

- **INFO** [unused-function] (line 116): Unused function: handleSaved
  ```
  function handleSaved() {
  ```
  💡 Remove if unused or document why kept

### `src/components/admin/generic/EntityCardSubPanels.vue` [P2] (score: 1)

- **INFO** [unused-function] (line 181): Unused function: togglePartsBulkEditMode
  ```
  const togglePartsBulkEditMode = () => {
  ```
  💡 Remove if unused or document why kept

### `src/components/admin/generic/fields/IconInput.vue` [P2] (score: 1)

- **INFO** [unused-function] (line 156): Unused function: openPicker
  ```
  const openPicker = () => {
  ```
  💡 Remove if unused or document why kept

### `src/components/booking/IndependentSelectCard.vue` [P2] (score: 1)

- **INFO** [unused-function] (line 122): Unused function: handleCheckboxClick
  ```
  function handleCheckboxClick(e: Event): void {
  ```
  💡 Remove if unused or document why kept

### `src/components/booking/SelectionCard.vue` [P2] (score: 1)

- **INFO** [unused-function] (line 179): Unused function: handleNumberUpdate
  ```
  const handleNumberUpdate = (value: string | number | null) => {
  ```
  💡 Remove if unused or document why kept

### `src/components/booking/SelectionCardGroup.vue` [P2] (score: 1)

- **INFO** [unused-function] (line 108): Unused function: handleNumberUpdate
  ```
  const handleNumberUpdate = (payload: { itemId: string; number: number | null }) => {
  ```
  💡 Remove if unused or document why kept

### `src/composables/fieldContext/useFieldContextState.ts` [P2] (score: 1)

- **INFO** [unused-function] (line 147): Unused function: validationRulesObject
  ```
  const validationRulesObject = (() => {
  ```
  💡 Remove if unused or document why kept

### `src/layouts/components/UserProfile.vue` [P2] (score: 1)

- **INFO** [unused-function] (line 19): Unused function: logout
  ```
  const logout = async () => {
  ```
  💡 Remove if unused or document why kept

### `src/main.ts` [P2] (score: 1)

- **INFO** [unused-function] (line 175): Unused function: prefetchGlobalData
  ```
  const prefetchGlobalData = async () => {
  ```
  💡 Remove if unused or document why kept

### `src/utils/transformers/appointmentToWizardTransformer.ts` [P2] (score: 1)

- **INFO** [unused-function] (line 133): Unused function: _findBlockInstanceByIdAndShapeName
  ```
  function _findBlockInstanceByIdAndShapeName(
  ```
  💡 Remove if unused or document why kept

### `src/views/admin/components/AnnotationTypeCard.vue` [P2] (score: 1)

- **INFO** [unused-function] (line 31): Unused function: handleEdit
  ```
  const handleEdit = () => {
  ```
  💡 Remove if unused or document why kept
