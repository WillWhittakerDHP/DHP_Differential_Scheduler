...
- **Requiring review: 2**
- Allowed (with justification): 1 (inline: 1, pattern: 0, specific: 0, linePattern: 0)
...
```
fieldMapping@243: onStatusChange({ appointmentId: record.id, oldStatus: 'submitted', newStatus: 'confirmed' }).catch((err) => {
fieldMapping@292: onStatusChange({ appointmentId: record.id, oldStatus, newStatus }).catch((err) => {
```
