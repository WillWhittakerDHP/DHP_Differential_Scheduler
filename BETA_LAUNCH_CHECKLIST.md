...
| Feature 9: Property Enrichment & Mappings | **Infrastructure Complete** | **Post-Beta** | Transformer, mappings, admin UI, client integration all built. Bright MLS API credentials not yet connected — provider requires beta launch before issuing access. |
| Feature 3: Booking Calculations | **Planning** | **Assess** | Fee/time calcs needed for real bookings. Pricing cascade tables + admin UI already built (undocumented in project manager). |
| Feature 4: Calendar & Appointment Availability | **Planning** | **Assess** | Server-side slot computation exists (`59ac037`). Calendar UI for booking flow not started. |
| Feature 6: GPT Admin Automation | **Planning** | No | Post-launch enhancement |
| Feature 7: UI Polish | **Planning** | **Partial** | "Good enough" for alpha, polish for beta |
| Beta Feedback System | **Complete** (undocumented) | Already built | Full system: model, API, UI widget, admin dashboard. Needs feature docs in `.project-manager/`. |
