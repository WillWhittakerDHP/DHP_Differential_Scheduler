...
describe('Relationship Router Integration Tests', () => {
  ...
  describe('GET /api/relationships/:relationshipKind', () => {
    it('should fetch all relationships of a kind', async () => {
      ...
      const mockFindAll = ActiveConstituent.findAll as jest.MockedFunction<() => Promise<RelationshipType[]>>
      mockFindAll.mockResolvedValue(mockRelationships)

      const response = await request(app)
        .get('/api/relationships/activeConstituents')
        .expect(200)

      expect(response.body).toEqual(mockRelationships)
      expect(ActiveConstituent.findAll).toHaveBeenCalled()
    })
    ...
    it('should handle fetch errors', async () => {
      const mockFindAll = ActiveConstituent.findAll as jest.MockedFunction<() => Promise<never>>
  ...
  describe('GET /api/relationships/:relationshipKind/:parentId/:childId', () => {
    it('should fetch relationship by parent and child IDs', async () => {
      ...
      const mockFindOne = ActiveConstituent.findOne as jest.MockedFunction<() => Promise<RelationshipType | null>>
      mockFindOne.mockResolvedValue(mockRelationship)

      const response = await request(app)
        .get('/api/relationships/activeConstituents/block-1/part-1')
        .expect(200)

      expect(response.body).toEqual(mockRelationship)
      expect(ActiveConstituent.findOne).toHaveBeenCalled()
    })
  ...
  describe('POST /api/relationships/:relationshipKind', () => {
    it('should create new relationship', async () => {
      ...
      const mockCreate = ActiveConstituent.create as jest.MockedFunction<() => Promise<RelationshipType>>
      ...
      const response = await request(app)
        .post('/api/relationships/activeConstituents')
        .send({
      ...
      expect(ActiveConstituent.create).toHaveBeenCalled()
    })
    ...
    it('should handle creation errors', async () => {
      const mockCreate = ActiveConstituent.create as jest.MockedFunction<() => Promise<never>>
      const mockFindByPk = BlockInstance.findByPk as jest.MockedFunction<(id: string) => Promise<{ id: string } | null>>
  ...
  describe('DELETE /api/relationships/:relationshipKind/:parentId/:childId', () => {
    it('should delete relationship', async () => {
      const mockRelationship: RelationshipType = { id: 'rel-1', parent_id: 'block-1', child_id: 'part-1' }
      const mockFindOne = ActiveConstituent.findOne as jest.MockedFunction<() => Promise<RelationshipType | null>>
      const mockDestroy = ActiveConstituent.destroy as jest.MockedFunction<() => Promise<number>>
      mockFindOne.mockResolvedValue(mockRelationship)
      mockDestroy.mockResolvedValue(1)

      await request(app)
        .delete('/api/relationships/activeConstituents/block-1/part-1')
        .expect(204)

      expect(ActiveConstituent.destroy).toHaveBeenCalled()
    })
