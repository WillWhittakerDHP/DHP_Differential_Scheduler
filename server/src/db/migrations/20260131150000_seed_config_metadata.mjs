/**
 * Migration: Seed metadata for config data types (events and annotations)
 * Date: 2026-01-31
 * Purpose: Seed metadata entries for annotationShape, annotationInstance, eventShape, and eventInstance
 *          These are configuration data types (not entities) that use configType='event' or 'annotation'
 * 
 * LEARNING: Config data metadata seeding
 * WHY: Metadata modals need field definitions in database to display fields for editing
 * PATTERN: Seed with configType='event' or 'annotation', configId=sentinel UUID, entityType/entityId=NULL
 */
...
export default {
  async up(queryInterface, Sequelize) {
    console.log('🔄 Seeding config data metadata (events and annotations)...')

    // Sentinel UUIDs for global configs
    // LEARNING: Separate UUIDs for shapes vs instances
    // WHY: Shapes and instances have different fields, so they need separate metadata configs
    // PATTERN: Use different sentinel UUIDs to distinguish shape vs instance metadata
    ...
    // WHY: Shapes and instances have different fields, so they need separate metadata configs
    // PATTERN: Seed shape fields with shape UUID, instance fields with instance UUID
  ...
  async down(queryInterface, Sequelize) {
    console.log('🔄 Reverting config data metadata seed...')
    ...
    console.log('✅ Reverted config data metadata seed')
  },
}
