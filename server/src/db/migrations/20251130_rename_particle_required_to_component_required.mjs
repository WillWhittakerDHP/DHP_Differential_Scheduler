...
export default {
  async up(queryInterface, Sequelize) {
    ...
    if (!tableExists) {
      console.log('ℹ️  Table block_instances does not exist, skipping');
      return;
    }
    
    // Check if column already exists
    // NOTE: describeTable returns snake_case keys (particle_required, component_required) for database columns
    const tableDescription = await queryInterface.describeTable('block_instances');
    
    // If particle_required exists and component_required doesn't, rename it
    if (tableDescription.particle_required && !tableDescription.component_required) {
    ...
    } else if (tableDescription.component_required) {
      ...
      try {
        ...
        // Column might already exist with different casing or name
        if (error.message && error.message.includes('already exists')) {
          console.log('ℹ️  Column component_required already exists, skipping');
        } else {
          throw error;
        }
      }
