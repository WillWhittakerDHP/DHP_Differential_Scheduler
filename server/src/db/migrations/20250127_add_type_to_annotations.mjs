...
export default {
  async up(queryInterface, Sequelize) {
    ...
    // Get frontPage type ID (should exist from seed data)
    const frontPageTypeResult = await queryInterface.sequelize.query(`
      SELECT id FROM annotation_types WHERE name = 'frontPage' LIMIT 1
    `, {
      type: Sequelize.QueryTypes.SELECT
    });

    const frontPageType = Array.isArray(frontPageTypeResult) 
      ? frontPageTypeResult[0] 
      : (frontPageTypeResult?.[0] || null);

    if (!frontPageType) {
      throw new Error('frontPage annotation type must exist. Run seed data first.');
    }

    const frontPageTypeId = frontPageType.id;
