...
- **Git / branch:** `/.cursor/commands/git/shared/tier-branch-manager.ts` (`ensureTierBranch`, `mergeTierBranch`, `commitUncommittedNonCursor` with branch check and allowed-prefix staging; see **Current implementation notes** for pre-checkout vs tier-end behavior)
- **Planning artifact recovery:** `/.cursor/commands/git/shared/artifact-branch-recovery.ts` (`recoverPlanningArtifactsAfterCheckout`; optional path filter via `autoCommittedPaths` from `EnsureTierBranchResult`)
- **Document I/O:** `/.cursor/commands/utils/document-manager.ts` (`readPlanningDoc`, `writePlanningDoc`, `planningDocExists`, `getPlanningDocRelativePath`, idempotent `appendLog`, `coalesceLogNormalization`; log coalesce entrypoint `/.cursor/commands/document/composite/coalesce-log.ts`)
...
- **Git — tier-end (`commitRemaining` / `commitUncommittedNonCursor`):** Before committing, the workflow verifies the current branch matches the tier’s expected branch (`getExpectedBranchForTier`). **Staged paths** are limited to allowed prefixes (default **`client/`**, **`server/`**, **`.project-manager/`** via `DEFAULT_ALLOWED_COMMIT_PREFIXES`), excluding **never-commit** paths (e.g. **`.cursor/`**, **`client/.audit-reports/`**, transient `.project-manager` dotfiles per `isNeverCommitPath`). On wrong branch, tier-end returns `wrong_branch_before_commit`.
