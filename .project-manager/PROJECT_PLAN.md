...
**Quality: harness vs raw agent (how we think about it)** — “Quality” isn’t one score. **Raw agent** use optimizes **time to the first edit** and feels great for tiny, obvious changes. **Harness** use optimizes **consistency** (same gates every time), **traceability** (plan → work → handoff/log), and **alignment** with architecture and audit checks. The tradeoff is **ceremony**: more steps before and after coding. We use a shorthand called **spec pressure**: when the work is small and obvious, harness and raw often converge; when the work is **cross-cutting or multi-session**, a written spec (planning doc + implementation orders) usually beats a long improvised thread—fewer wrong files and fewer “we fixed it in chat but nobody wrote it down” gaps.

**The honest brag** — The harness doesn’t make the model smarter. It makes the **system around the model** intentional: bounded context, reason-coded outcomes, and recovery paths when git or audits fail. Alpha isn’t “look at our AI”—it’s “look at how we keep AI-assisted work **shippable across sessions**.”
