# Session 8.7.1 Guide: ** Ownership registry + real `checkOwnership` middleware (models, owner fields, 403/404, logging)
...
**Session ID:** 8.7.1
**Session Name:** Ownership registry + real `checkOwnership` middleware (models, owner fields, 403/404, logging)
**Description:** Replace the `checkOwnership` stub with a registry-backed implementation so CRUD and custom routes enforce row ownership using `req.user.id`.
...
- `server/src/middlewares/security.ts` (imports only until task 8.7.1.2)
...
- `server/src/middlewares/ownershipRegistry.ts` (if split helpers are needed)
**Approach:** Async middleware using registry; align error body shape with existing `requireRole` / auth patterns where practical.
**Checkpoint:** Manual spot-check: wrong user gets 403 on at least one wired route; server lint clean.
...
**Recommended:** Use `/session-start 8.7.1 [description]` to automatically:
...
```
## Session: 8.7.1 - [Brief Description]
**Date:** [Date]
...
**Task Format (illustrative — use real task ids in Quick Start):**
```
#### Task 8.7.1.N: <short title>
**Goal:** <specific objective>
**Files:** 
- <paths>
**Approach:** <how you will implement>
