# Session 8.8.1 Guide: ** Create Joi schemas for User, PropertyFieldMapping, and PropertyFeatureMapping models; wire `validateRequest` callbacks into all three CRUD router configs; run server lint; update GC-8-JOI checklist
...
**Session ID:** 8.8.1
**Session Name:** ** Create Joi schemas for User, PropertyFieldMapping, and PropertyFeatureMapping models; wire `validateRequest` callbacks into all three CRUD router configs; run server lint; update GC-8-JOI checklist
...
**Approach:** Import schemas from task 1; add `validateRequest: (req, method) => { ... }` callback per CRUD config following `betaFeedbackCrudRouter` pattern. Update GC-8-JOI status and notes. Lint + verify server starts.
**Checkpoint:** All three CRUD configs have `validateRequest`; lint passes; server starts; checklist updated
...
**Recommended:** Use `/session-start 8.8.1 [description]` to automatically:
...
```
## Session: 8.8.1 - [Brief Description]
**Date:** [Date]
