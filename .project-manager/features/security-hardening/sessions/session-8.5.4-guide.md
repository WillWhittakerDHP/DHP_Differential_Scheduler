...
**Session ID:** 8.5.4
**Session Name:** Joi gap closure batch B — Audit remaining server/src/routes/internal routers for missing validateRequest; same constraints as 8.5.3; close or narrow GC-8-JOI when all targeted mutating routes are covered or explicitly exempted with documented rationale. Consult GAP_CLOSURE_HARNESS_ADD_PROMPTS.md and then update GAP_CLOSURE_CHECKLIST GC-8-JOI when batch is verified (lint + smoke).
...
- All router files under mounts 12–17 (read-only)
...
- `server/src/routes/internal/property-mappings/propertyMappingsRouter.ts` (modify)
...
**Approach:** Run verification checks. Write combined batch summary. Update checklist row. Update session docs.
**Checkpoint:** GC-8-JOI is `done` with evidence. App starts; lint passes.
...
**Recommended:** Use `/session-start 8.5.4 [description]` to automatically:
...
```
## Session: 8.5.4 - [Brief Description]
**Date:** [Date]
