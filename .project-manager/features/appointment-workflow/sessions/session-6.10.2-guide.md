...
- **Coupon list and Coupons block shape:** When we wire or show the list of coupons (e.g. Apply Coupon dropdown or future cascade), **reuse the block-shape-filtered instances strategy** used for service, property, and option. For those, the wizard uses `getBlockShapeIdByType(bookingData, BLOCK_SHAPE_TYPES.SERVICE)` etc. (`client/src/utils/blockInstanceUtils.ts`), then filters `bookingData.blockInstances` by `blockShapeRef === shapeId`. For Coupons, the shape is the user-created "Coupons" block shape (by name); resolve it (e.g. `bookingData.blockShapes.find(bs => bs.name === 'Coupons')` or a helper `getBlockShapeIdByName(bookingData, 'Coupons')`), then filter instances the same way. See phase 6.10 guide and `cascadeFilterPipeline.ts` / `useWizardFilteredOptions` for the pattern.
