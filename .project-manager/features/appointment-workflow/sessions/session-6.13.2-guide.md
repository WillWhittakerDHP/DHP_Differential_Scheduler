# Session 6.13.2 Guide: ** `BookingWizard.vue` / `BookingWizard.scss` integration; brand + quote/reschedule verification; client lint
...
**Session ID:** 6.13.2
**Session Name:** ** `BookingWizard.vue` / `BookingWizard.scss` integration; brand + quote/reschedule verification; client lint
...
- `client/src/components/booking/BookingWizard.scss` — `.dhp-colors-active` block, `.quote-mode-active` / `.reschedule-mode-active` blocks, default CSS custom properties
- `client/src/composables/useThemeMode.ts` — touch-ups only if `on-*` format or var contract needs adjustment
**Approach:** 
- Update both class-binding sites in BookingWizard.vue (root VCard ~line 93 and stepper-header ~line 106) to apply `quote-mode-active` / `reschedule-mode-active` regardless of `useDhpColors`
- Remove or empty the hardcoded `--v-theme-*` declarations inside `.dhp-colors-active` that override the pipeline (yellow/red DHP triplets); the composable already sets these at `:root`
- Review whether `.quote-mode-active` / `.reschedule-mode-active` SCSS blocks still need their own `--v-theme-*` overrides or can also defer to the composable; simplify where possible
- Keep `--inactive-*` defaults that have no pipeline equivalent unless the pipeline now covers them
**Checkpoint:** 
- Class bindings apply mode + brand simultaneously when both are active
...
- `client/src/components/booking/BookingWizard.scss`
- `client/src/composables/useThemeMode.ts`
**Approach:** 
- Open the wizard in the browser; toggle brand on/off and cycle through new, quote, and reschedule modes
- Verify button colors, stepper header backgrounds, inactive step colors, and on-* contrast for each combination
- Run `cd client && npm run lint` and fix any lint errors in touched files
- Verify app starts cleanly with `npm run start:dev`
**Checkpoint:** 
- All six combinations produce visually correct, distinct output
- No regression to admin brand toggle behavior
...
**Recommended:** Use `/session-start 6.13.2 [description]` to automatically:
...
```
## Session: 6.13.2 - [Brief Description]
**Date:** [Date]
