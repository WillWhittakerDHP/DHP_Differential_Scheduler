# Session 20.6.2 Guide: EntityCard tree and façade consumers
...
**Session ID:** 20.6.2
**Session Name:** EntityCard tree and façade consumers
**Description:** Replace every **`EntityCard.vue`** consumer with domain-named parents that compose **`EntityCardContent`** and existing composables; remove **`AnnotationShapeListCard`’s** wrap of **`EntityCard`**; then delete the **`EntityCard*`** SFC tree and orphan **`useEntityCard*`** modules after **`RelationshipCollection`** no longer async-imports **`EntityCard`**.

**Duration:** Large (multi-task session)
**Status:** In Progress (planning filled — await **`/accepted-plan`**)

### Tasks

- [x] #### Task 20.6.2.1: List surfaces, modals, and annotation façade
**Goal:** Remove **`EntityCard`** from **`BlockInstancesGroup`**, **`ShapesTabEventPanel`**, **`ShapesTabPartPanel`**, **`ShapeCardList`**, **`ShapeCreationForm`**, **`BulkEditModal`**, **`BlockInstanceCreateModal`**; refactor **`AnnotationShapeListCard`** to stop wrapping **`EntityCard.vue`** while preserving expansion + field editing.
**Files:**
- `client/src/views/admin/tabs/components/BlockInstancesGroup.vue`, `ShapesTabEventPanel.vue`, `ShapesTabPartPanel.vue`, `ShapeCardList.vue`, `ShapeCreationForm.vue`, `BulkEditModal.vue`, `BlockInstanceCreateModal.vue`
- `client/src/components/admin/generic/AnnotationShapeListCard.vue`
- Shared inner pieces: `EntityCardContent.vue`, `EntityCardSubPanels.vue`, etc. (read/compose — delete in 20.6.2.2)
...
- `EntityCard*.vue`, `composables/admin/useEntityCard*.ts`, `types/admin/entityCard*.ts`, `utils/admin/entityCard*.ts` (as orphaned)
**Approach:** Same inner composition pattern as 20.6.2.1 for nested rows; then mechanical delete + grep verification.
**Checkpoint:** No **`EntityCard.vue`**; inventory doc + worklog updated; lint/typecheck/start per DoD.
...
**Recommended:** Use `/session-start 20.6.2 [description]` to automatically:
...
```
## Session: 20.6.2 - [Brief Description]
**Date:** [Date]
