# Session 20.3.1 Guide: Placement type editor (FEATURE_20 §8.3 #1)
...
**Session ID:** 20.3.1
**Session Name:** Placement type editor (§8.3 #1)
...
- `client/src/configs/field/display/appliedDisplay/eventShapeDisplays.ts` (as needed for labels)
...
**Approach:** Rewrite labels/help; matrix row secondary line explains placement-derived effect without leading with “differential role.”
**Checkpoint:** Lint/typecheck clean; manual read of Shapes → Event shapes + block instance override matrix.
...
**Recommended:** Use `/session-start 20.3.1 [description]` to automatically:
...
```
## Session: 20.3.1 - [Brief Description]
**Date:** [Date]
...
**Task Format:**
```
#### Task 20.3.1.N: Example task title
...
**Goal:** Clear, specific objective (replace when copying this template).

**Files:** 
- Source: `path/to/source` (if porting/migrating)
- Target: `path/to/target` (if creating new)
