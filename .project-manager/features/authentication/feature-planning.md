...
- **Server:** `server/src/**` — models/migrations aligned with Phase 10.1; auth config, session handling, middleware, and route protection in Phase 7.2–7.3; strategy implementations (magic link first).
...
## Approach
1. **Follow the guide’s phase order:** `10.1` → `7.2` → `7.3` → `7.4`; treat `7.5` as deferred until magic-link + client paths are stable and product agrees on password/OAuth scope.
...
3. **Open questions:** In **7.2–7.4**, decide or stub **tester user-type switching** (minimal dev-only affordance vs full matrix) and record **Google OAuth** as out-of-scope for initial beta unless explicitly pulled into a phase.
...
## Checkpoint
- **After 10.1:** Data model and migrations support users/sessions (or agreed equivalents); no blocking schema gaps for magic link.
