...
- `/feature-start [name]` - Start feature (create branch, load context, checkpoint)
  - **CRITICAL:** Creates `feature-plan.md` and `README.md` FIRST, before any phase planning
  - See "Feature Creation Workflow Order" section for detailed sequence
...
- `/plan-phase [N] [description]` - Plan new phase
  - **VALIDATION:** Checks that feature-level docs (`feature-plan.md`, `README.md`) exist and are complete
  - Blocks phase planning if feature docs incomplete
...
- `/feature-create [name] [description]` - Create feature structure
  - **MUST create `feature-plan.md` and `README.md` before any phase planning**
  - Creates feature directory: `project-manager/features/[feature-name]/`
  - Creates `feature-plan.md` with template structure (overview, objectives, phase breakdown, success criteria, dependencies)
  - Creates `README.md` with basic overview (status, description, objectives, phase list)
  - Initializes optional guide/log/handoff files if needed
- `/feature-research [name]` - Conduct research phase (mandatory, 30+ questions)
  - **MUST complete before phase planning**
  - Documents findings in feature guide/log
  - Updates `feature-plan.md` with research insights
...
- `/phase-create [N] [description]` - Create phase structure
  - **VALIDATION:** MUST check that feature-level docs (`feature-plan.md`, `README.md`) exist and are complete before creating phase docs
  - Blocks phase creation if feature docs incomplete
  - Creates `phases/` subdirectory if it doesn't exist
  - Creates phase-specific docs (guide, log, handoff)
...
- `/feature-start` = `/feature-create` + `/feature-research` + `/feature-load` + git branch + `/feature-checkpoint`
  - **CRITICAL ORDER:** Must create feature-level planning documents (`feature-plan.md`, `README.md`) BEFORE any phase planning
  - Feature structure creation must complete before phase planning can begin
  - See "Feature Creation Workflow" section below for detailed order
...
#### Feature Creation Workflow Order

**CRITICAL:** Feature-level planning documents MUST be created before any phase planning documents. This maintains the hierarchical structure: Feature → Phase → Session → Task.

**Required Order of Operations:**

1. **Feature Structure Creation** (`/feature-create`):
   - Create feature directory: `project-manager/features/[feature-name]/`
   - Create `feature-plan.md` with template structure:
     - Feature overview and objectives
     - Phase breakdown (high-level, not detailed phase plans)
     - Success criteria
     - Dependencies
     - Key files and architecture notes
   - Create `README.md` with basic overview:
     - Feature status and description
     - Key objectives
     - Phase list (references, not full plans)
     - Related documents
   - Initialize optional guide/log/handoff files if needed

2. **Research Phase** (`/feature-research`):
   - Conduct 30+ question research covering 6 categories
   - Document findings in feature guide/log
   - Update `feature-plan.md` with research insights

3. **Feature Planning Completion**:
   - Ensure `feature-plan.md` has complete phase breakdown (high-level)
   - Ensure `README.md` references `feature-plan.md`
   - Verify all feature-level docs are complete

4. **Validation Before Phase Planning**:
   - Check that `feature-plan.md` exists and has content
   - Check that `README.md` exists and has content
   - Verify feature plan includes phase breakdown (high-level)
   - Verify research phase completed (if required)
   - Verify feature directory structure created

5. **Only Then: Phase Planning**:
   - After feature-level docs are complete and validated, allow phase planning
   - Phase planning creates `phases/` subdirectory
   - Phase planning creates phase-specific docs

**Validation:** Phase planning commands (`/plan-phase`, `/phase-create`) MUST check for feature-level docs and block phase creation if feature docs are incomplete.
...
- **Purpose:** Ensure feature-level planning documents are complete before phase planning
- **Validation Document:** See `FEATURE_VALIDATION_CHECKLIST.md` for complete validation requirements
- **When Validates:** Before any phase planning commands (`/plan-phase`, `/phase-create`)
- **What Validates:** 
  - `feature-plan.md` exists and has required sections
  - `README.md` exists and has required sections
  - Feature directory structure is correct
  - Research phase completed (if required)
- **Behavior:** Blocks phase creation if validation fails, provides clear error messages
...
Agent: 
1. Creates feature directory: project-manager/features/vue-migration/
2. Creates feature-plan.md with template structure (overview, objectives, phase breakdown)
3. Creates README.md with basic overview
4. Presents research question set (30+ questions)
5. Guides user through research phase
6. Documents research findings in feature guide/log
7. Updates feature-plan.md with research insights
8. Creates optional feature guide, log, and handoff files

User: /feature-start vue-migration

Agent:
1. Validates feature-plan.md exists and has content
2. Validates README.md exists and has content
3. Validates feature plan includes phase breakdown
4. Creates git branch: feature/vue-migration
5. Loads feature context
6. Creates initial checkpoint
7. Prepares for work
8. NOW allows phase planning (only after feature docs complete)
```
...
Agent:
1. VALIDATES: Checks feature-plan.md exists and has content
2. VALIDATES: Checks README.md exists and has content
3. VALIDATES: Verifies feature plan includes phase breakdown
4. If validation fails: Blocks phase creation, shows error message
5. If validation passes: Creates phases/ subdirectory if needed
6. Creates phase structure (guide, log, handoff)
7. Loads phase context
