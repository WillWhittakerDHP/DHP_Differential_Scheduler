...
- **Time orchestrator composition parity (2026-07-20):** fixed `availableComposablePeersForComposer` so composite packages pick same-shape *atomics* (not other packages); stopped booking extraction from requiring child `composite=true`; Composite toggle no longer hidden when off; migration `079` marks time-shape orchestrators composite so Property Details packages get the Components panel like service packages; server `validateBlockInstancesCompositeForComponents` now requires only the parent to be composite (atomic children allowed).
- **Accumulation atomic-only (2026-07-20):** Accumulator + Accumulation links only on atomic service cards; link picker offers atomic time blocks only; server rejects composite parents/children; runtime ignores composite accumulator parents; migration `080` soft-disables package-owned accumulation links (e.g. Blue Tape).
